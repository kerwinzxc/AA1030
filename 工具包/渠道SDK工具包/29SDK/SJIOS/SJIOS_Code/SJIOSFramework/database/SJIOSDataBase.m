...
- (NSString *)dataFilePath{
    ...
    return [documentsDirectory stringByAppendingPathComponent:@"SJIOS.db"];
...
- (BOOL) createSJIOSList:(sqlite3*)db {
    ...
    //这句是大家熟悉的SQL语句
    char *sql = "create table if not exists SJIOSuserinfo(ID INTEGER PRIMARY KEY AUTOINCREMENT, username text,password text, bind text,tourist text,logintime text, phonenumber text)";
...
-(BOOL) insertSJIOSList:(SJIOSUserInfo *)insertList {
    ...
    if ([self openDB]) {
        ...
        static char *sql = "INSERT INTO SJIOSuserinfo(username, password,bind,tourist,logintime,phonenumber) VALUES(?, ?, ?, ?, ?, ?)";
...
- (NSMutableArray*)getTestList{
    ...
    if ([self openDB]) {
        ...
        //sql语句
        char *sql = "SELECT *FROM SJIOSuserinfo";//从testTable这个表中获取 testID, testValue ,testName，若获取全部的话可以用*代替testID, testValue ,testName。
...
-(BOOL)updateSJIOSUserInfo:(SJIOSUserInfo*)updateSJIOSUserInfo{
    if ([self openDB]) {
        ...
        //组织SQL语句
        char *sql = "update SJIOSuserinfo set  logintime = ? , password = ? WHERE username = ?";
...
-(BOOL) updateSJIOSTourist:(SJIOSUserInfo *)updateList{
    ...
    if ([self openDB]) {
        sqlite3_stmt *statement;//这相当一个容器，放转化OK的sql语句
        //组织SQL语句
        char *sql = "update SJIOSuserinfo set username = ? , password = ? ,bind = ?, logintime = ? ,phonenumber = ? WHERE tourist = ?";
...
-(BOOL) updateSJIOSPassword:(NSString *)password userName:(NSString *)username
{
    if ([self openDB]) {
        sqlite3_stmt *statement;//这相当一个容器，放转化OK的sql语句
        //组织SQL语句
        char *sql = "update SJIOSuserinfo set  password = ? WHERE username = ?";
...
- (BOOL) deleteSJIOSList:(NSString *)deletList{
    if ([self openDB]) {
        sqlite3_stmt *statement;
        //组织SQL语句
        static char *sql = "delete from SJIOSuserinfo  where username = ?";
        //将SQL语句放入sqlite3_stmt中
        int success = sqlite3_prepare_v2(_database, sql, -1, &statement, NULL);
        if (success != SQLITE_OK) {
...
- (BOOL)searchSJIOSUser:(NSString*)searchString{
    ...
    if ([self openDB]) {
        sqlite3_stmt *statement = nil;
        //sql语句
        NSString *querySQL = [NSString stringWithFormat:@"SELECT * from SJIOSuserinfo where username = ?"];
...
- (BOOL)searchSJIOSBindUser:(NSString *)searchString{
    ...
    if ([self openDB]) {
        sqlite3_stmt *statement = nil;
        //sql语句
        NSString *querySQL = [NSString stringWithFormat:@"SELECT * from SJIOSuserinfo where tourist = ?"];
        const char *sql = [querySQL UTF8String];
        //        char *sql = "SELECT * FROM testTable WHERE testName like ?";//这里用like代替=可以执行模糊查找，原来是"SELECT * FROM testTable WHERE testName = ?"
        if (sqlite3_prepare_v2(_database, sql, -1, &statement, NULL) != SQLITE_OK) {
