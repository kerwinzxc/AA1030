...
#import "SJIOSForgetView.h"
#import "SJIOSBaseView.h"
#define SJIOS_USER_AGREEMENT_URL @"http://www.3975.com/public/web/39XY.html"

@interface SJIOSYMLoginView()
...
@property(nonatomic,strong)  SJIOSRegisterView *registerView;//注册
@property(nonatomic,strong) SJIOSForgetView *forgetPhoneView;//手机找回密码界面
@property(nonatomic) Boolean SHOWPROCESS;
@property(nonatomic,strong) SJIOSBaseView *baseView;
@property(nonatomic,strong) SJIOSYMLoginView *ymLoginView;//登录界面
...
@implementation SJIOSYMLoginView
...
-(void)setLandScapeView{
    ...
    UITextField * username = [[UITextField alloc] initWithFrame:CGRectMake(50, 98, 310, 38)];
    ...
    UITextField * password = [[UITextField alloc] initWithFrame:CGRectMake(50, 148, 310, 38)];
    ...
    //登录界面的立即注册label
    UILabel *login_register = [[UILabel alloc] initWithFrame:CGRectMake(360, 108, 20, 38)];
    ...
    //登录界面的忘记密码label
    UILabel *ym_fogetPassword = [[UILabel alloc] initWithFrame:CGRectMake(360, 158, 20, 38)];
...
-(void)setPortraitView{
    ...
    //登录界面的立即注册label
    UILabel *login_register = [[UILabel alloc] initWithFrame:CGRectMake(30, 220, 20, 38)];
    [login_register setText:@"注册帐号"];
    login_register.textColor = [UIColor blueColor];
    [login_register sizeToFit];
    UITapGestureRecognizer* registTapGestureRecognizer = [[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(ymRegister)];
    ...
    //登录界面的忘记密码label
    UILabel *ym_fogetPassword = [[UILabel alloc] initWithFrame:CGRectMake(160, 220, 20, 38)];
    [ym_fogetPassword setText:@"忘记密码" ];
    ym_fogetPassword.textColor = [UIColor blueColor];
    [ym_fogetPassword sizeToFit];
    [ym_fogetPassword addGestureRecognizer:[[UITapGestureRecognizer alloc]initWithTarget:self action:@selector(fogetPassword)]];
...
//登录框
- (void) YMloginPressed:(UIButton*)sender
{
...
-(void)ymStartLogin{
        ...
        @try{
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if(self.textIsphoneNumber){
                    [[SJIOSSdkImp sharedInstance] addSJIOSUser:username password:password bind:@"1" tourist:@"0" phoneNumber:@"1"];
                }else{
                    [[SJIOSSdkImp sharedInstance] addSJIOSUser:username password:password bind:@"0" tourist:@"0" phoneNumber:@"0"];
                }
    ...
    SJIOS_VSD_BLOCK getTokenCompletionBlock = ^(NSDictionary *dictionary, NSDictionary *headers){
        @try{
            ...
            if ((NSNull *)value == nil)
            {
                ...
                [[SJIOSCommonApi sharedInstance] getSJIOSUserId:access_token
    ...
    SJIOS_VSD_BLOCK completionBlock = ^(NSDictionary *dictionary, NSDictionary *headers) {
        ...
        @try
        {
            ...
            if ((NSNull *)value == nil)
            {
                ...
                [[SJIOSCommonApi sharedInstance] getSJIOSAccessToken:[dictionary valueForKey:@"code"]
    ...
    SJIOS_VSD_BLOCK completionBlock1 = ^(NSDictionary *dictionary, NSDictionary *headers) {
        // self.logining = false;
        @try
        {
            if (dictionary == nil)
            ...
            NSLog(@"YMLoginView  completionBlock1 ::dictionary==%@" ,dictionary);
    ...
    if(self.textIsphoneNumber){//手机号登录
        ...
        [[SJIOSCommonApi sharedInstance] phoneSJIOSLogin:username
                                          password:password
                                   completionBlock:completionBlock1 failedBlock:^(NSError * error) {
                                       self.logining = false;
                                       self.failed(-109);
                                       [[SJIOSSdkImp sharedInstance]showSJIOSToast:@"请检查网络连接！"];
                                   }];
        ...
        [[SJIOSCommonApi sharedInstance] login:username
                                     password:[SJIOSSdkMd5 md5:password]
...
- (void) startLogin
{
        ...
        @try{
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if(self.isRegister == YES){
                    [[SJIOSSdkImp sharedInstance] showSJIOSShiMingRenZhengView:self.token callback:^(NSString *callbackxxxx){
                        NSString * userid = [dictionary valueForKey:@"id"];
                        ...
                        [[SJIOSSdkImp sharedInstance] addSJIOSUser:self.loginInfo.username password:self.loginInfo.password bind:self.loginInfo.bind    tourist:self.loginInfo.tourist phoneNumber:self.loginInfo.phoneNumber];
                        [self accountClick];
    ...
    //普通注册后的普通登录
        [[SJIOSCommonApi sharedInstance] login:self.loginInfo.username
...
-(void) ymRegister{
    ...
    if (self.registerView == nil) {
        ...
        self.registerView = [[SJIOSRegisterView alloc] initWithBlock:^(NSString * username, NSString * password) {
...
-(void)fogetPassword
{
    ...
    if (self.forgetPhoneView == nil) {
        self.forgetPhoneView = [[SJIOSForgetView alloc] initWithLandscape:self.landscape];
    }
