...
@property(nonatomic,strong) SJIOSForgetView *ymForgetView;//忘记密码找回界面
...
@implementation SJIOSForgetView
...
-(void)setLandScapeView{
    ...
    UIImage *shengsubackImage = [[UIImage alloc]initWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"SJIOS.bundle/SJIOS_login_button.png"]] ;
...
-(void)ym_shensuForgetClick
{
    SJIOS_VSD_BLOCK successBlock= ^(NSDictionary *dictionary, NSDictionary *header) {
        ...
        if ([dictionary objectForKey:@"error"] == nil) {
            ...
            SJIOSForgetEmailView *forgetView = [[SJIOSForgetEmailView alloc]initWithLandscape:self.landscape phone:self.phone qq:self.qq];
    ...
    [[SJIOSCommonApi sharedInstance]getSJIOSSupport:[[SJIOSSdkImp sharedInstance]appId] onSuccess:successBlock onFailed:^(NSError *error) {
        [[SJIOSCommonApi sharedInstance]getSJIOSSupport:[[SJIOSSdkImp sharedInstance]appId] onSuccess:successBlock onFailed:^(NSError *error) {
...
- (void)verifyPressed
{
    ...
    [[SJIOSCommonApi sharedInstance] changeSJIOSPasswordByPhone:username
...
//                process.labelText = [[SJIOSSdkImp sharedInstance] translateSJIOS:[dictionary valueForKey:@"error"]];
        ...
        [[SJIOSCommonApi sharedInstance] confirmchangeSJIOSPasswordByPhone:username
