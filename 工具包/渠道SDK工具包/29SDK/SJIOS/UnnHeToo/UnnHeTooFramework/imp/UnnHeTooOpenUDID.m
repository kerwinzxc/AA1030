...
@interface UnnHeTooOpenUDID (Private)
...
@implementation UnnHeTooOpenUDID
...
+ (NSString*) value {
    return [UnnHeTooOpenUDID valueWithError:nil];
}
...
+ (NSString*) valueWithError:(NSError **)error {
    ...
    for (int n=0; n<kOpenUDIDRedundancySlots; n++) {
        ...
        if (slotPB==nil) {
            ...
            NSDictionary* dict = [UnnHeTooOpenUDID _getDictFromPasteboard:slotPB];
    ...
    if (openUDID==nil) {        
        if (mostReliableOpenUDID==nil) {
            ...
            openUDID = [UnnHeTooOpenUDID _generateFreshOpenUDID];
        } else {
    ...
    if (availableSlotPBid!=nil && (myRedundancySlotPBid==nil || [availableSlotPBid isEqualToString:myRedundancySlotPBid])) {
        ...
        if (openUDID && localDict)
            [UnnHeTooOpenUDID _setDict:localDict forPasteboard:slotPB];
    }
