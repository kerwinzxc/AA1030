...
#import  "UnnHeTooSdkImp.h"
@interface UnnHeTooBindIdentifyViewController ()
...
@property(nonatomic,strong) UnnHeTooBindIdentifyViewController *bindIdentifyVew;


@end

@implementation UnnHeTooBindIdentifyViewController
...
-(void)initNavigation {
    ...
    serviceImageView.image = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"UnnHeToo.bundle/UnnHeToo_icon_meun_shiming.png"]];
...
- (void)normalPressed
{
    ...
    if (verify.length == 0)
    {
        ...
        [self showUnnHeTooToast:@"请输入身份证号码"];
    ...
    if (verify.length != 18)
    {
        ...
        [self showUnnHeTooToast:@"身份证号码有误"];
        ...
        @try
        {
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if (ok != nil)
                {
                    ...
                    [self showUnnHeTooToast:@"认证成功，请返回"];
                ...
                [self showUnnHeTooToast:@"身份证信息有误"];
                return;
            }
            else
            {
                
                [self showUnnHeTooToast:[[UnnHeTooSdkImp sharedInstance] translateUnnHeToo:[dictionary valueForKey:@"error"]]];
            }
        }
        @catch(NSException * e)
        {
    ...
    [[UnnHeTooCommonApi sharedInstance] startUnnHeTooIdentify:self.accessToken name:identifyNameField.text identifyNumber:identifyNumberField.text completionBlock:completionBlock failedBlock:^(NSError *error) {
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    ...
    else if(textField==identifyNumberField)
    {
        ...
        if ([username length] == 0)
        {
            ...
            [[UnnHeTooSdkImp sharedInstance]showUnnHeTooToast:@"姓名为空"];
        ...
        {
            [[UnnHeTooSdkImp sharedInstance]showUnnHeTooToast:@"身份证为空"];
