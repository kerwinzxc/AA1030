...
@interface UnnHeTooRegisterView()<UIAlertViewDelegate>
...
@property(nonatomic,strong) UnnHeTooRegisterView *ymRegisterView;//帐号注册界面

@end

@implementation UnnHeTooRegisterView
...
-(void)setLandScapeView{
    ...
    username.background = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"UnnHeToo.bundle/UnnHeToo_register_edit_background.png"]];
    ...
    secondpassword.background = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"UnnHeToo.bundle/UnnHeToo_register_edit_background.png"]];
    ...
    UIImage *logo_p=[UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"UnnHeToo.bundle/UnnHeToo_logo_s.png"]];
    ...
    [registerButton setBackgroundImage:[UIImage imageNamed:@"UnnHeToo.bundle/UnnHeToo_login_button.png"] forState:UIControlStateNormal];
...
- (void)normalPressed
{
    ...
    {
        [[UnnHeTooSdkImp sharedInstance]showUnnHeTooToast:@"用户名中不能有空格"];
        return;
    }
    else
    {
    ...
    if (ret == YES) {
        ...
        [[UnnHeTooSdkImp sharedInstance]showUnnHeTooToast:@"账户不能为纯数字"];
    ...
    if ([username length] <6 || [username length] > 20) {
          ...
          [[UnnHeTooSdkImp sharedInstance]showUnnHeTooToast:@"账户长度为6到20位"];
            return;
    }
    ...
    if (![password isEqualToString:self.secondPassword.text]) {
        [[UnnHeTooSdkImp sharedInstance]showUnnHeTooToast:@"对不起你两次输入的密码不匹配"];
        return;
    }
    ...
    if(![pred evaluateWithObject: self.username.text])
    {
        ...
        [[UnnHeTooSdkImp sharedInstance] showUnnHeTooToast: @"用户名只能由字母+数字组成,并且字母在数字前面"];
        return;
    }
    ...
    UnnHeToo_VSD_BLOCK completionBlock = ^(NSDictionary *dictionary, NSDictionary *headers) {
        ...
        @catch(NSException * e)
        {
            [[UnnHeTooSdkImp sharedInstance]showUnnHeTooToast:@"内部错误,请重新注册"];
            self.registering = false;
        }
        @finally
        {
    ...
    [[UnnHeTooCommonApi sharedInstance] normalUnnHeTooRegister:username
...
-(void)process {
    if (self.myProcess == nil) {
        self.myProcess = [[UnnHeTooProgressHUD alloc] initWithView:self.alertContainer];
    }
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    if (textField == self.username){
        ...
        if ([username length] == 0)
        {
            ...
            [[UnnHeTooSdkImp sharedInstance]showUnnHeTooToast:@"账户为空"];
        ...
        {
            [[UnnHeTooSdkImp sharedInstance]showUnnHeTooToast:@"重复密码为空"];
