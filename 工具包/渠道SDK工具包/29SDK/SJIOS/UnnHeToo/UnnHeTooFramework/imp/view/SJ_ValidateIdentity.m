...
-(void)setLandScapeView{
    ...
    [close setBackgroundImage:[UIImage imageNamed:@"UnnHeToo.bundle/UnnHeToo_ym_close.png"] forState:UIControlStateNormal];
    [close addTarget:self action:@selector(closeUnnHeTooPressed) forControlEvents:UIControlEventTouchUpInside];
    ...
    self.name.background = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"UnnHeToo.bundle/UnnHeToo_register_edit_background.png"]];
    ...
    self.identity.background = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"UnnHeToo.bundle/UnnHeToo_register_edit_background.png"]];
...
-(void) comfirmPressed{
    ...
    if([self.identity.text length] != 18){
        [[UnnHeTooSdkImp sharedInstance]showUnnHeTooToast:@"身份证号码输入有误"];
        return;
    }
        ...
        @try
        {
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if (ok != nil)
                {
                    ...
                    [[UnnHeTooSdkImp sharedInstance] showUnnHeTooToast:@"认证成功"];
                    self.complete(@"ok", @"1");
                    [self dismissWithAnimation:YES];
                ...
                {
                    [[UnnHeTooSdkImp sharedInstance] showUnnHeTooToast:@"信息有误，认证失败"];
    ...
    [[UnnHeTooCommonApi sharedInstance] startUnnHeTooIdentify:self.accesstoken name:[self.name.text stringByReplacingOccurrencesOfString:@" " withString:@""] identifyNumber:[self.identity.text stringByReplacingOccurrencesOfString:@" " withString:@""] completionBlock:completionBlock failedBlock:^(NSError *error) {
