...
@interface UnnHeTooYMPhoneRegister()
...
@implementation UnnHeTooYMPhoneRegister
...
-(void)setLandScapeView{
    ...
    UIImage * logo_r = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"UnnHeToo.bundle/UnnHeToo_logo_s.png"]];
...
-(void)ym_verifyPressed:(id)sender {
    ...
    UnnHeToo_VSD_BLOCK completionBlock = ^(NSDictionary *dictionary, NSDictionary *headers) {
        ...
        @try
        {
            ...
            if ((NSNull *)value == nil)
            {
                ...
                [[UnnHeTooSdkImp sharedInstance]showUnnHeTooToast:@"获取验证码成功"];
                self.logining = false;
            ...
            else
            {
                ...
                self.myProcess.labelText = [[UnnHeTooSdkImp sharedInstance] translateUnnHeToo:[dictionary valueForKey:@"error"]];
                self.myProcess.mode = UnnHeTooProgressHUDModeCustomView;
    ...
    [[UnnHeTooCommonApi sharedInstance] phoneUnnHeTooRegistVerify:self.ymPhoneNumber
...
//                [self setRandomAccount];
    ...
    [[UnnHeTooCommonApi sharedInstance] phoneUnnHeTooRegist:self.ymPhoneNumber
