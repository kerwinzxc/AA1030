...
@property(nonatomic,strong) UnnHeTooBind *ymBind;//绑定正式帐号界面

@end


@implementation UnnHeTooBind
...
-(void) image:(UIImage *)image didFinishSavingResult:(NSError *)error contextInfo:(void *)contentInfo
{
    if(error == nil){
        ...
        [[UnnHeTooSdkImp sharedInstance] showUnnHeTooToast:@"绑定成功！帐号信息截图已保存到相册中，帐号丢失可以查看截图找回，请妥善保管！"];
...
- (void)bindPressed
{
    ...
    if(![pred evaluateWithObject: self.username.text])
    {
        ...
        [[UnnHeTooSdkImp sharedInstance] showUnnHeTooToast: @"用户名只能由字母跟数字组成"];
        return;
    }
    ...
    [[UnnHeTooCommonApi sharedInstance] bindUnnHeTooRegister:username
                                          password:[UnnHeTooSdkMd5 md5:password]
                                    bindUsername:self.bindname
