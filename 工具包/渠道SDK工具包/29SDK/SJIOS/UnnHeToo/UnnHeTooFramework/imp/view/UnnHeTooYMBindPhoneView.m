...
#import "UnnHeTooYMBindPhoneView.h"
...
@interface UnnHeTooYMBindPhoneView()
...
@property(nonatomic,strong) UnnHeTooYMBindPhoneView *ymBindPhoneView;//忘记密码找回界面
...
@implementation UnnHeTooYMBindPhoneView
...
-(void)setLandScapeView{
    ...
    [closeButton setBackgroundImage:[UIImage imageNamed:@"UnnHeToo.bundle/UnnHeToo_ym_close.png"] forState:UIControlStateNormal];
...
{
    UnnHeTooForgetEmailView *forgetView = [[UnnHeTooForgetEmailView alloc]initWithLandscape:self.landscape];
...
- (void)verifyPressed
{
    ...
    if (phone.length == 0)
    {
        ...
        [[UnnHeTooSdkImp sharedInstance]showUnnHeTooToast:@"请输入手机号"];
        return;
    }
    ...
    if(self.textIsphoneNumber){
        ...
        [[UnnHeTooCommonApi sharedInstance] bindUnnHeTooPhone:self.accessToken
                                            phone:phone
...
- (void)normalPressed
{
    ...
    if (username.length == 0)
    {
        ...
        [[UnnHeTooSdkImp sharedInstance]showUnnHeTooToast:@"手机号或者密码为空"];
        return;
    }
    ...
    if (verify.length == 0)
    {
        [[UnnHeTooSdkImp sharedInstance]showUnnHeTooToast:@"请输入验证码"];
        return;
...
//                    [self checkAccountIdentity:self.accessToken];//检查有没有实名
    ...
    [[UnnHeTooCommonApi sharedInstance] confirmUnnHeTooPhone:self.accessToken
                                          verify:verify
                                 completionBlock:completionBlock
                                     failedBlock:^(NSError * error) {
                                    UnnHeTooProgressHUD * process = [[UnnHeTooProgressHUD alloc] initWithView:self.alertContainer];
