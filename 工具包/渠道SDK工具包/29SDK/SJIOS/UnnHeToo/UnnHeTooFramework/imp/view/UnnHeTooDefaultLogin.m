...
//@property (nonatomic,strong) UnnHeTooUserInfo *phoneLoginInfo;
...
//用户列表新增==========
@property(nonatomic, strong)UnnHeTooDataBase *UnnHeTooDatabase;//数据库
//初始化数据库，查询用户列表并显示
...
@implementation UnnHeTooDefaultLogin
...
- (id)initWithBlock:(UnnHeToo_SUCCESS_Callback)success failed:(UnnHeToo_FAILED_Callback)failed landScape:(Boolean)landcape {
    ...
    self.UnnHeTooDatabase = [[UnnHeTooDataBase alloc]init];
    ...
    if (self) {
        ...
        if (self.UnnHeTooDatabase) {
            
            self.alluserinfo=[self.UnnHeTooDatabase getTestList];
        }else{
...
-(void)setLandScapeView{
    ...
    if([_alluserinfo count] > 0){
        ...
        UnnHeTooUserInfo *userinfo=[self.dataList objectAtIndex:0];
    ...
    //self.defaultButton.backgroundColor=[UIColor grayColor];
    [self.defaultButton setBackgroundImage:[UIImage imageNamed:@"UnnHeToo.bundle/UnnHeToo_register_edit_background.png"] forState:UIControlStateNormal];
    ...
    self.arrow.image=[UIImage imageNamed:@"UnnHeToo.bundle/UnnHeToo_arrow.png"];
    ...
    [loginButton setBackgroundImage:[UIImage imageNamed:@"UnnHeToo.bundle/UnnHeToo_login_button.png"] forState:UIControlStateNormal];
    ...
    [_registButton setBackgroundImage:[UIImage imageNamed:@"UnnHeToo.bundle/UnnHeToo_login_button.png"] forState:UIControlStateNormal];
    ...
    if ([self.loginInfo.tourist isEqual:@"1"]&&[self.loginInfo.bind isEqual:@"0"]) {
        ...
        [self.defaultButton setTitle:[NSString stringWithFormat:@"@快速登录%@",username] forState:UIControlStateNormal];
...
- (void) startLogin
{
    ...
    if([self.loginInfo.password isEqualToString:@"0"]){
        ...
        //if (self.ymLoginView==nil) {
            self.ymLoginView=[[UnnHeTooYMLoginView alloc]initWithBlock:self.success failed:self.failed landScape:self.landscape view:self username:self.loginInfo.username];
        //}
    ...
    if ([self.loginInfo.tourist  isEqual:@"1"]) {
        ...
        password=[UnnHeTooSdkMd5 md5:self.loginInfo.password];
    }
    ...
    UnnHeToo_VSD_BLOCK getUserIdCompletionBlock = ^(NSDictionary *dictionary, NSDictionary *headers){
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if(self.isRegister == YES){
                    [[UnnHeTooSdkImp sharedInstance] showUnnHeTooShiMingRenZhengView:self.token callback:^(NSString *callbackssss){
                        ...
                        [[UnnHeTooSdkImp sharedInstance] addUnnHeTooUser:self.loginInfo.username password:self.loginInfo.password bind:self.loginInfo.bind    tourist:self.loginInfo.tourist phoneNumber:self.loginInfo.phoneNumber];
        ...
        {
            //[[UnnHeTooSdkImp sharedInstance]showUnnHeTooToast:@"登录失败，请退出重新进游戏" ];
            self.login = NO;
        }
        @finally
        {
    ...
    UnnHeToo_VSD_BLOCK getTokenCompletionBlock = ^(NSDictionary *dictionary, NSDictionary *headers){
        ...
        {
            [[UnnHeTooSdkImp sharedInstance]showUnnHeTooToast:@"获取信息失败，请退出重新进游戏" ];
            self.login = NO;
    ...
    UnnHeToo_VSD_BLOCK completionBlock = ^(NSDictionary *dictionary, NSDictionary *headers) {
        ...
        @catch(NSException * e)
        {
            ...
            [[UnnHeTooSdkImp sharedInstance] showUnnHeTooToast:@"验证失败，请退出重新进游戏"];
            self.login = NO;
    ...
    if([self.loginInfo.phoneNumber isEqual:@"1"]){
        ...
        [[UnnHeTooCommonApi sharedInstance] phoneUnnHeTooLogin:self.loginInfo.username
                                          password:password
    ...
    [[UnnHeTooCommonApi sharedInstance] login:self.loginInfo.username
                                 password:password
...
- (void) registerPressed
{
    ...
    if ([self.loginInfo.tourist isEqual:@"1"]&&[self.loginInfo.bind isEqual:@"0"]) {
        ...
        if(self.bindview==nil){
            self.bindview=[[UnnHeTooBind alloc]initWithBlock:^(NSString *username, NSString *password) {
                UnnHeTooUserInfo *userinfo=[[UnnHeTooUserInfo alloc]initWithInfo:username password:password bind:@"1" tourist:@"0" loginTime:0 phoneNumber:@"0"];
                self.loginInfo=userinfo;
                [self startLogin];
        ...
        [[UnnHeTooSdkImp sharedInstance] removeUnnHeTooUser:self.loginInfo.username];//绑定后删掉快速登录帐号
        
    }else{
        
        
          if (self.registerView == nil) {
        self.registerView = [[UnnHeTooRegisterView alloc] initWithBlock:^(NSString * username, NSString * password) {
            UnnHeTooUserInfo *userinfo=[[UnnHeTooUserInfo alloc]initWithInfo:username password:password bind:@"0" tourist:@"0" loginTime:0 phoneNumber:@"0"];
            self.loginInfo=userinfo;
            self.isRegister = YES;
            [self startLogin];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UnnHeTooUserInfo *info=[self.dataList objectAtIndex:row];
    if ([info.tourist isEqual:@"1"]&&[info.bind isEqual:@"0"]) {
         NSString *username=[info.username substringToIndex:6];
        cell.textLabel.textColor=[UIColor redColor];
        cell.textLabel.text = [NSString stringWithFormat:@"@快速登录%@",username];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    UnnHeTooUserInfo *info=[self.dataList objectAtIndex:[indexPath row]];
    self.loginInfo=info;
...
-(void) tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSUInteger row=[indexPath row];
    UnnHeTooUserInfo *info=[self.dataList objectAtIndex:row];
    [[UnnHeTooSdkImp sharedInstance] removeUnnHeTooUser:info.username];
    ...
    if (self.dataList.count>0) {
        ...
        [self dismissWithAnimation:YES];
        [[UnnHeTooSdkImp sharedInstance]loginUnnHeToo:[UnnHeTooSdkImp sharedInstance].rootControllerView landscape:self.landcape success:self.success failed:self.failed flag:1];
    }
