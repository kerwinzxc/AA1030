...
#import  "UnnHeTooSdkImp.h"
@interface UnnHeTooBindEmailViewController ()
...
@implementation UnnHeTooBindEmailViewController
...
-(void)initNavigation {
    ...
    serviceImageView.image = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"UnnHeToo.bundle/UnnHeToo_icon_meun_bangemail.png"]];
...
- (void)normalPressed
{
    ...
    UnnHeToo_VSD_BLOCK completionBlock = ^(NSDictionary *dictionary, NSDictionary *headers) {
     ...
     [[UnnHeTooCommonApi sharedInstance] bindUnnHeTooMail:self.accessToken email:verify completionBlock:completionBlock failedBlock:^(NSError *error) {
         [[UnnHeTooCommonApi sharedInstance] bindUnnHeTooMail:self.accessToken email:verify completionBlock:completionBlock failedBlock:^(NSError *error) {
             [[UnnHeTooCommonApi sharedInstance] bindUnnHeTooMail:self.accessToken email:verify completionBlock:completionBlock failedBlock:^(NSError *error) {
                 if (self.process == nil) {
                     self.process = [[UnnHeTooProgressHUD alloc] initWithView:self.view];
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    if (textField == self.phoneNumberField)
    {
        ...
        if ([username length] == 0)
        {
            ...
            [[UnnHeTooSdkImp sharedInstance]showUnnHeTooToast:@"邮箱为空"];
