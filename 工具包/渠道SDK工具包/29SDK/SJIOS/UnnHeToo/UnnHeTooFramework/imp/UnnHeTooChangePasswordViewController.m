...
#import "UnnHeTooSdkImp.h"
@interface UnnHeTooChangePasswordViewController ()
...
@implementation UnnHeTooChangePasswordViewController
...
-(void)initNavigation {
    ...
    serviceImageView.image = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"UnnHeToo.bundle/UnnHeToo_icon_meun_change_pw.png"]];
...
-(void)changePassword {
        ...
        @try
        {
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if([userinfo.tourist isEqualToString:@"1"]){
                    ...
                    nowPassword=[UnnHeTooSdkMd5 md5:self.nowPasswordField.text];
                }else{
                    
                    nowPassword=self.nowPasswordField.text;
                }
                
                [[[UnnHeTooDataBase alloc]init]updateUnnHeTooPassword:nowPassword userName:userinfo.username];
    ...
    if([userinfo.tourist isEqualToString:@"1"]){
        ...
        oldPassword = [UnnHeTooSdkMd5 md5:self.oldPasswordField.text];
    }
    
    [[UnnHeTooCommonApi sharedInstance]changeUnnHeTooPasswordWithToken:self.accessToken oldPassword:oldPassword newPassword:[UnnHeTooSdkMd5 md5:newPassword] completionBlock:completionBlock failedBlock:^(NSError *a) {
...
-(void)bindDespatch {
    ...
    [process showAnimated:YES whileExecutingBlock:^{
        ...
        while (self.changing)
        {
            ...
            [NSThread sleepForTimeInterval:1];
            if (count > 5) {
                UnnHeTooProgressHUD *errorHud = [[UnnHeTooProgressHUD alloc] initWithView:self.view];
