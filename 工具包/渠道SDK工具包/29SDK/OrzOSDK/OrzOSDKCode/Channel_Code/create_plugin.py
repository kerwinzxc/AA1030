...
def filetext_replace(filepath, plugin_name):
    ...
    with open(filepath, "r") as f:
        filecontent = f.read()
        filecontent = re.sub(r'AppleContainer_Test', plugin_name, filecontent)
...
def rename_plugin(plugin_folder, plugin_name):
    for root, dirs, filenames in os.walk(plugin_folder):
        for filename in filenames:
            newfilename = re.sub("AppleContainer_Test", plugin_name, filename)
    ...
    for root, dirs, filenames in os.walk(plugin_folder, topdown=False):
        for dirname in dirs:
            newdirname = re.sub("AppleContainer_Test", plugin_name, dirname)
...
def create_plugin(plugin_root, plugin_name, template_type):
    if not plugin_name.startswith('AppleContainer_'):
        plugin_name = 'AppleContainer_' + plugin_name

    new_plugin_path = os.path.join(plugin_root, plugin_name)
    template_plugin_path = os.path.join(plugin_root, "AppleContainer_Test")
        #    if template_type and template_type == 'simple':
        #template_plugin_path = os.path.join(plugin_root, "U8SDK_SimpleTemplate")
