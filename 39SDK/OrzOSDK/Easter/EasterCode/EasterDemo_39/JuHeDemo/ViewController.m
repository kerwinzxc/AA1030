...
@interface ViewController()<EasterDelegate>
...
-(void)action_init{
    [Easter getSharedInstance].EasterDelegate = self;
    [[Easter getSharedInstance] initEaster];
...
-(void)action_login{
    ...
    [[Easter getSharedInstance] startEasterLogin];
}

-(void)action_logout{
    [[Easter getSharedInstance] startEasterLogout];
}

-(void)action_showfloat{
    [[Easter getSharedInstance] showEasterFloatView];
}

-(void)action_hidefloat{
    [[Easter getSharedInstance] hideEasterFloatView];
...
-(void)action_pay{
    ...
    [data setValue:@"60yuanbao" forKey:Easter_IOS_PRODUCT_NAME];//必传，商品名称
    [data setValue:@"6" forKey:Easter_IOS_PRODUCT_ID];//必传，商品id，由运营提供
    [data setValue:orderId forKey:Easter_IOS_CP_ORDERID];//必传，CP订单号，若无，则传时间戳
    [data setValue:@"Easter_IOS_PRODUCT_DESC" forKey:Easter_IOS_PRODUCT_DESC];//必传，若无，则传商品名称
    [data setValue:@"6" forKey:Easter_IOS_PRODUCT_PRICE];//必传，商品价格，单位：元,不带小数点
    [data setValue:@"1" forKey:Easter_IOS_GOODS_NUM];//必传，商品购买数量；一般传1
    [data setValue:@"Easter_IOS_EXTRA" forKey:Easter_IOS_EXTRA];//可选，扩展信息，若无，则传空
    [data setValue:@"Easter_IOS_ROLE_ID" forKey:Easter_IOS_ROLE_ID];//可选，角色id，若无，则传空
    [data setValue:@"Easter_IOS_ROLE_NAME" forKey:Easter_IOS_ROLE_NAME];//可选，角色名称，若无，则传空
    [data setValue:@"Easter_IOS_ROLE_LEVEL" forKey:Easter_IOS_ROLE_LEVEL];//可选，角色等级，若无，则传空
    [data setValue:@"Easter_IOS_SERVER_ID" forKey:Easter_IOS_SERVER_ID];//可选，服务器id，若无，则传空
    [data setValue:@"Easter_IOS_SERVER_NAME" forKey:Easter_IOS_SERVER_NAME];//可选，服务器名称，若无，则传空
    [[Easter getSharedInstance] startEasterRegcharg:data];
    
}

-(void)action_createrole{
    NSMutableDictionary * data = [NSMutableDictionary dictionary];
    [data setValue:@"1" forKey:Easter_ROLE_ID];//必传，角色ID
    [data setValue:@"1" forKey:Easter_ROLE_LEVEL];//必传，角色等级
    [data setValue:@"roleName" forKey:Easter_ROLE_NAME];//必传，角色名称
    [data setValue:@"roleCreateTime" forKey:Easter_ROLE_CREATE_TIME];//必传，角色创建时间，时间戳，精确到秒
    [data setValue:@"1" forKey:Easter_SERVER_ID];//必传，服务器id
    [data setValue:@"serverName" forKey:Easter_SERVER_NAME];//必传，服务器名称
    [[Easter getSharedInstance] sendEasterData:Easter_SUBMIT_ROLE_CREATE data:data];
}
...
-(void)action_rolelogin{
    ...
    [data setValue:@"1" forKey:Easter_SERVER_ID];//必传，服务器id
    [data setValue:@"serverName" forKey:Easter_SERVER_NAME];//必传，服务器名称
    [[Easter getSharedInstance] sendEasterData:Easter_SUBMIT_ROLE_ENTERSERVER data:data];
}
...
-(void)action_levelup{
    ...
    [data setValue:@"1" forKey:Easter_SERVER_ID];//必传，服务器id
    [data setValue:@"serverName" forKey:Easter_SERVER_NAME];//必传，服务器名称
    [[Easter getSharedInstance] sendEasterData:Easter_SUBMIT_ROLE_LEVELUP data:data];
}
