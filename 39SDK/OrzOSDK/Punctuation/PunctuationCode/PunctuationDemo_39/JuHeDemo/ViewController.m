...
@interface ViewController()<PunctuationDelegate>
...
-(void)action_init{
    [Punctuation getSharedInstance].PunctuationDelegate = self;
    [[Punctuation getSharedInstance] initPunctuation];
...
-(void)action_login{
    ...
    [[Punctuation getSharedInstance] startPunctuationLogin];
}

-(void)action_logout{
    [[Punctuation getSharedInstance] startPunctuationLogout];
}

-(void)action_showfloat{
    [[Punctuation getSharedInstance] showPunctuationFloatView];
}

-(void)action_hidefloat{
    [[Punctuation getSharedInstance] hidePunctuationFloatView];
...
-(void)action_pay{
    ...
    [data setValue:@"60yuanbao" forKey:Punctuation_IOS_PRODUCT_NAME];//必传，商品名称
    [data setValue:@"6" forKey:Punctuation_IOS_PRODUCT_ID];//必传，商品id，由运营提供
    [data setValue:orderId forKey:Punctuation_IOS_CP_ORDERID];//必传，CP订单号，若无，则传时间戳
    [data setValue:@"Punctuation_IOS_PRODUCT_DESC" forKey:Punctuation_IOS_PRODUCT_DESC];//必传，若无，则传商品名称
    [data setValue:@"6" forKey:Punctuation_IOS_PRODUCT_PRICE];//必传，商品价格，单位：元,不带小数点
    [data setValue:@"1" forKey:Punctuation_IOS_GOODS_NUM];//必传，商品购买数量；一般传1
    [data setValue:@"Punctuation_IOS_EXTRA" forKey:Punctuation_IOS_EXTRA];//可选，扩展信息，若无，则传空
    [data setValue:@"Punctuation_IOS_ROLE_ID" forKey:Punctuation_IOS_ROLE_ID];//可选，角色id，若无，则传空
    [data setValue:@"Punctuation_IOS_ROLE_NAME" forKey:Punctuation_IOS_ROLE_NAME];//可选，角色名称，若无，则传空
    [data setValue:@"Punctuation_IOS_ROLE_LEVEL" forKey:Punctuation_IOS_ROLE_LEVEL];//可选，角色等级，若无，则传空
    [data setValue:@"Punctuation_IOS_SERVER_ID" forKey:Punctuation_IOS_SERVER_ID];//可选，服务器id，若无，则传空
    [data setValue:@"Punctuation_IOS_SERVER_NAME" forKey:Punctuation_IOS_SERVER_NAME];//可选，服务器名称，若无，则传空
    [[Punctuation getSharedInstance] startPunctuationRegcharg:data];
    
}

-(void)action_createrole{
    NSMutableDictionary * data = [NSMutableDictionary dictionary];
    [data setValue:@"1" forKey:Punctuation_ROLE_ID];//必传，角色ID
    [data setValue:@"1" forKey:Punctuation_ROLE_LEVEL];//必传，角色等级
    [data setValue:@"roleName" forKey:Punctuation_ROLE_NAME];//必传，角色名称
    [data setValue:@"roleCreateTime" forKey:Punctuation_ROLE_CREATE_TIME];//必传，角色创建时间，时间戳，精确到秒
    [data setValue:@"1" forKey:Punctuation_SERVER_ID];//必传，服务器id
    [data setValue:@"serverName" forKey:Punctuation_SERVER_NAME];//必传，服务器名称
    [[Punctuation getSharedInstance] sendPunctuationData:Punctuation_SUBMIT_ROLE_CREATE data:data];
}
...
-(void)action_rolelogin{
    ...
    [data setValue:@"1" forKey:Punctuation_SERVER_ID];//必传，服务器id
    [data setValue:@"serverName" forKey:Punctuation_SERVER_NAME];//必传，服务器名称
    [[Punctuation getSharedInstance] sendPunctuationData:Punctuation_SUBMIT_ROLE_ENTERSERVER data:data];
}
...
-(void)action_levelup{
    ...
    [data setValue:@"1" forKey:Punctuation_SERVER_ID];//必传，服务器id
    [data setValue:@"serverName" forKey:Punctuation_SERVER_NAME];//必传，服务器名称
    [[Punctuation getSharedInstance] sendPunctuationData:Punctuation_SUBMIT_ROLE_LEVELUP data:data];
}
