...
@property(nonatomic,strong) RersistenceBind *ymBind;//绑定正式帐号界面

@end


@implementation RersistenceBind
...
-(void) image:(UIImage *)image didFinishSavingResult:(NSError *)error contextInfo:(void *)contentInfo
{
    if(error == nil){
        ...
        [[RersistenceSdkImp sharedInstance] showRersistenceToast:@"绑定成功！帐号信息截图已保存到相册中，帐号丢失可以查看截图找回，请妥善保管！"];
...
- (void)bindPressed
{
    ...
    if(![pred evaluateWithObject: self.username.text])
    {
        ...
        [[RersistenceSdkImp sharedInstance] showRersistenceToast: @"用户名只能由字母跟数字组成"];
        return;
    }
    ...
    [[RersistenceWebInterface sharedInstance] bindRersistenceRegister:username
                                          passwordRersistence:[RersistenceSdkMd5 md5:password]
                                    bindUsernameRersistence:self.bindname
