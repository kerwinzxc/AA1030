...
-(void)setLandScapeView{
    ...
    [close setBackgroundImage:[UIImage imageNamed:@"Rersistence.bundle/Rersistence_ym_close.png"] forState:UIControlStateNormal];
    [close addTarget:self action:@selector(closeRersistencePressed) forControlEvents:UIControlEventTouchUpInside];
    ...
    self.name.background = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Rersistence.bundle/Rersistence_register_edit_background.png"]];
    ...
    self.identity.background = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Rersistence.bundle/Rersistence_register_edit_background.png"]];
...
-(void) comfirmPressed{
    ...
    if([self.identity.text length] != 18){
        [[RersistenceSdkImp sharedInstance]showRersistenceToast:@"身份证号码输入有误"];
        return;
    }
        ...
        @try
        {
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if (ok != nil)
                {
                    ...
                    [[RersistenceSdkImp sharedInstance] showRersistenceToast:@"认证成功"];
                    self.complete(@"ok", @"1");
                    [self dismissWithAnimation:YES];
                ...
                {
                    [[RersistenceSdkImp sharedInstance] showRersistenceToast:@"信息有误，认证失败"];
    ...
    [[RersistenceWebInterface sharedInstance] startRersistenceIdentify:self.accesstoken nameRersistence:[self.name.text stringByReplacingOccurrencesOfString:@" " withString:@""] identifyNumberRersistence:[self.identity.text stringByReplacingOccurrencesOfString:@" " withString:@""] successCallbackRersistence:successCallback failCallbackRersistence:^(NSError *error) {
