...
#import "RersistenceForgetEmailView.h"


@interface RersistenceForgetView()
...
@property(nonatomic,strong) RersistenceForgetView *ymForgetView;//忘记密码找回界面
...
@implementation RersistenceForgetView
...
-(void)setLandScapeView{
    ...
    [bindPhoneFindPasswordButton setBackgroundImage:[UIImage imageNamed:@"Rersistence.bundle/Rersistence_login_button.png"] forState:UIControlStateNormal];
    ...
    [shensuFindPasswordButton setBackgroundImage:[UIImage imageNamed:@"Rersistence.bundle/Rersistence_login_button.png"] forState:UIControlStateNormal];
...
//    [shensuFindPasswordButton addSubview:imageView];
    ...
    accountField.background = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Rersistence.bundle/Rersistence_register_edit_background.png"]];
    ...
    verifyField.background = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Rersistence.bundle/Rersistence_register_edit_background.png"]];
    ...
    [verifyButton setBackgroundImage:[UIImage imageNamed:@"Rersistence.bundle/Rersistence_login_button.png"] forState:UIControlStateNormal];
    ...
    newPasswordField.background = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Rersistence.bundle/Rersistence_register_edit_background.png"]];
   ...
   // [newPasswordField setValue:[UIColor whiteColor] forKeyPath:@"_placeholderLabel.textColor"];
    ...
    UIImage *surebackImage = [[UIImage alloc]initWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Rersistence.bundle/Rersistence_login_button.png"]] ;
...
-(void)setPortraitView{
    ...
    [sureButton setBackgroundImage:[UIImage imageNamed:@"Rersistence.bundle/Rersistence_login_button.png"] forState:UIControlStateNormal];
    [sureButton addTarget:self action:@selector(normalPressed) forControlEvents:UIControlEventTouchUpInside];
    sureButton.zhw_ignoreEvent = NO;//yes不启用防止重复点击，NO启用防止重复点击
    sureButton.zhw_acceptEventInterval = 3.0;//重复点击间隔时间
    [self.alertContainer addSubview:sureButton];
    self.alertContainer.backgroundColor=[UIColor colorWithPatternImage:[UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Rersistence.bundle/Rersistence_background_landscape.png"]]];
...
{
    Rersistence_VSD_BLOCK successBlock= ^(NSDictionary *dictionary, NSDictionary *header) {
        ...
        if ([dictionary objectForKey:@"error"] == nil) {
            ...
            RersistenceForgetEmailView *forgetView = [[RersistenceForgetEmailView alloc]initWithLandscape:self.landscape phone:self.phone qq:self.qq];
    ...
    [[RersistenceWebInterface sharedInstance]getRersistenceSupport:[[RersistenceSdkImp sharedInstance]getRersistenceAppId] onSuccessRersistence:successBlock onFailedRersistence:^(NSError *error) {
...
- (void)verifyPressed
{
    ...
    {
        [[RersistenceSdkImp sharedInstance]showRersistenceToast:@"手机号为空"];
        return;
    ...
    Rersistence_VSD_BLOCK successCallback = ^(NSDictionary *dictionary, NSDictionary *headers) {
        ...
        @try
        {
            ...
            if ((NSNull *)value == nil)
            {
                ...
                RersistenceProgressHUD * process = [[RersistenceProgressHUD alloc] initWithView:self.alertContainer];
            ...
            else
            {
                ...
                process.labelText = [[RersistenceSdkImp sharedInstance] translateRersistence:[dictionary valueForKey:@"error"]];
                process.mode = RersistenceProgressHUDModeCustomView;
                process.customView = [[UIImageView alloc] initWithImage:[UIImage imageNamed:@"Checkmark"]];
                [process showAnimated:YES whileExecutingBlock:^{
                     [NSThread sleepForTimeInterval:1];
    ...
    [[RersistenceWebInterface sharedInstance] changeRersistencePasswordByPhone:username
...
- (void)normalPressed
{
    ...
    if (username.length == 0)
    {
        RersistenceProgressHUD *myProcess = [[RersistenceProgressHUD alloc] initWithView:self.alertContainer];
        
        [self.alertContainer addSubview:myProcess];
        //process.dimBackground = YES;
        myProcess.mode = RersistenceProgressHUDModeCustomView;
...
//                process.labelText = [[RersistenceSdkImp sharedInstance] translateRersistence:[dictionary valueForKey:@"error"]];
        ...
        [[RersistenceWebInterface sharedInstance] confirmchangeRersistencePasswordByPhone:username
                                                            passwordRersistence:[RersistenceSdkMd5 md5:password]
                                                              verifyRersistence:verify
                                                     successCallbackRersistence:successCallback
                                                         failCallbackRersistence:^(NSError * error) {
                                                             self.reseting = NO;
                                                         }
...
//    }
    ...
    [process showAnimated:YES whileExecutingBlock:^{
        ...
        while (self.reseting)
        {
            ...
            [NSThread sleepForTimeInterval:1];
            if (count > 5) {
                RersistenceProgressHUD *errorHud = [[RersistenceProgressHUD alloc] initWithView:self.alertContainer];
                [self.alertContainer addSubview:errorHud];
                errorHud.labelText = @"网络不太好，请重试！！！";
                errorHud.mode = RersistenceProgressHUDModeCustomView;
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    if (textField == self.username){
        ...
        if ([verify length] == 0)
        {
            ...
            [[RersistenceSdkImp sharedInstance]showRersistenceToast:@"验证码为空"];
