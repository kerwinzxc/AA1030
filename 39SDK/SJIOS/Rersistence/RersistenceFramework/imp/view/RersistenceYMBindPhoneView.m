...
#import "RersistenceYMBindPhoneView.h"
...
@interface RersistenceYMBindPhoneView()
...
@property(nonatomic,strong) RersistenceYMBindPhoneView *ymBindPhoneView;//忘记密码找回界面
...
@implementation RersistenceYMBindPhoneView
...
-(void)setLandScapeView{
    ...
    [closeButton setBackgroundImage:[UIImage imageNamed:@"Rersistence.bundle/Rersistence_ym_close.png"] forState:UIControlStateNormal];
    ...
    UIImage *verifybackImage = [[UIImage alloc]initWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Rersistence.bundle/Rersistence_login_button.png"]] ;
...
{
    RersistenceForgetEmailView *forgetView = [[RersistenceForgetEmailView alloc]initWithLandscape:self.landscape];
...
- (void)verifyPressed
{
    ...
    if (phone.length == 0)
    {
        ...
        [[RersistenceSdkImp sharedInstance]showRersistenceToast:@"请输入手机号"];
        return;
    }
    ...
    if(self.textIsphoneNumber){
        ...
        [[RersistenceWebInterface sharedInstance] bindRersistencePhone:self.accessToken
                                            phoneRersistence:phone
...
- (void)normalPressed
{
    ...
    if (username.length == 0)
    {
        ...
        [[RersistenceSdkImp sharedInstance]showRersistenceToast:@"手机号或者密码为空"];
        return;
    }
    ...
    if (verify.length == 0)
    {
        [[RersistenceSdkImp sharedInstance]showRersistenceToast:@"请输入验证码"];
        return;
...
//                    [self checkAccountIdentity:self.accessToken];//检查有没有实名
    ...
    [[RersistenceWebInterface sharedInstance] confirmRersistencePhone:self.accessToken
