...
#import  "RersistenceSdkImp.h"
@interface RersistenceBindEmailViewController ()
...
@implementation RersistenceBindEmailViewController
...
-(void)initNavigation {
    ...
    serviceImageView.image = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Rersistence.bundle/Rersistence_icon_meun_bangemail.png"]];
...
- (void)normalPressed
{
    ...
    Rersistence_VSD_BLOCK successCallback = ^(NSDictionary *dictionary, NSDictionary *headers) {
     ...
     [[RersistenceWebInterface sharedInstance] bindRersistenceMail:self.accessToken emailRersistence:verify successRersistenceCallback:successCallback failRersistenceCallback:^(NSError *error) {
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    if (textField == self.phoneNumberField)
    {
        ...
        if ([username length] == 0)
        {
            ...
            [[RersistenceSdkImp sharedInstance]showRersistenceToast:@"邮箱为空"];
