...
#import  "RersistenceSdkImp.h"
@interface RersistenceBindIdentifyViewController ()
...
@property(nonatomic,strong) RersistenceBindIdentifyViewController *bindIdentifyVew;


@end

@implementation RersistenceBindIdentifyViewController
...
-(void)initNavigation {
    ...
    serviceImageView.image = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Rersistence.bundle/Rersistence_icon_meun_shiming.png"]];
...
- (void)normalPressed
{
    ...
    if(identifyNameField.text.length == 0){
        [self showRersistenceToast:@"请输入姓名"];
        return;
    }
    ...
    if (verify.length == 0)
    {
        ...
        [self showRersistenceToast:@"请输入身份证号码"];
    ...
    if (verify.length != 18)
    {
        ...
        [self showRersistenceToast:@"身份证号码有误"];
        ...
        @try
        {
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if (ok != nil)
                {
                    ...
                    [self showRersistenceToast:@"认证成功，请返回"];
                    [self dismissViewControllerAnimated:YES completion:^{
                    }];
                    ...
                    [self showRersistenceToast:@"认证失败"];
                ...
                [self showRersistenceToast:@"身份证信息有误"];
                return;
            }
            else
            {
                
                [self showRersistenceToast:[[RersistenceSdkImp sharedInstance] translateRersistence:[dictionary valueForKey:@"error"]]];
            }
        }
        @catch(NSException * e)
        {
    ...
    [[RersistenceWebInterface sharedInstance] startRersistenceIdentify:self.accessToken nameRersistence:identifyNameField.text identifyNumberRersistence:identifyNumberField.text successCallbackRersistence:successCallback failCallbackRersistence:^(NSError *error) {
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    ...
    else if(textField==identifyNumberField)
    {
        ...
        if ([username length] == 0)
        {
            ...
            [[RersistenceSdkImp sharedInstance]showRersistenceToast:@"姓名为空"];
        ...
        {
            [[RersistenceSdkImp sharedInstance]showRersistenceToast:@"身份证为空"];
