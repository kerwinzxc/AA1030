...
#import "RersistenceSdkImp.h"
@interface RersistenceChangePasswordViewController ()
...
@implementation RersistenceChangePasswordViewController
...
-(void)initNavigation {
    ...
    serviceImageView.image = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Rersistence.bundle/Rersistence_icon_meun_change_pw.png"]];
...
-(void)changePassword {
    ...
    if ([newPassword length] == 0) {
        RersistenceProgressHUD *process = [[RersistenceProgressHUD alloc] initWithView:self.view];
        ...
        @try
        {
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if([userinfo.tourist isEqualToString:@"1"]){
                    ...
                    nowPassword=[RersistenceSdkMd5 md5:self.nowPasswordField.text];
                }else{
                    
                    nowPassword=self.nowPasswordField.text;
                }
                
                [[[RersistenceDataBase alloc]init]updateRersistencePassword:nowPassword userName:userinfo.username];
    ...
    if([userinfo.tourist isEqualToString:@"1"]){
        ...
        oldPassword = [RersistenceSdkMd5 md5:self.oldPasswordField.text];
    }
    
    [[RersistenceWebInterface sharedInstance]changeRersistencePasswordWithTokenRersistence:self.accessToken oldPasswordRersistence:oldPassword newPasswordRersistence:[RersistenceSdkMd5 md5:newPassword] successCallbackRersistence:successCallback failCallbackRersistence:^(NSError *a) {
...
-(void)bindDespatch {
    ...
    [process showAnimated:YES whileExecutingBlock:^{
        ...
        while (self.changing)
        {
            ...
            [NSThread sleepForTimeInterval:1];
            if (count > 5) {
                RersistenceProgressHUD *errorHud = [[RersistenceProgressHUD alloc] initWithView:self.view];
