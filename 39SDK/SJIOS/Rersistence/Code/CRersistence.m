...
@interface CRersistence ()

@end

@implementation CRersistence
...
- (instancetype)init {
    ...
    for (NSString *countryCode in countryCodes) {
        ...
        CRersistence *metaData = [[CRersistence alloc]init];
        NSLog(@"%@", metaData);
        if (metaData) {
...
- (CRersistence *)getMetadataForRegion:(NSString *)regionCode {
     return nil;
}
...
- (NSString *)inputDigitHelper_:(NSString *)nextChar {
    ...
    if ([NB_REGION_CODE_FOR_NON_GEO_ENTITY isEqualToString:numberWithoutCountryCallingCode]) {
        CRersistence *helper = [[CRersistence alloc] init];
