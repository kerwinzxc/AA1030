...
@interface UsequenceRegisterView()<UIAlertViewDelegate>
...
@property(nonatomic,strong) UsequenceRegisterView *ymRegisterView;//帐号注册界面

@end

@implementation UsequenceRegisterView
...
-(void)setLandScapeView{
    ...
    username.background = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Usequence.bundle/Usequence_register_edit_background.png"]];
    ...
    UIImage * logo_r = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Usequence.bundle/Usequence_logo_r.png"]];
    ...
    secondpassword.background = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Usequence.bundle/Usequence_register_edit_background.png"]];
    ...
    UIImage *logo_p=[UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Usequence.bundle/Usequence_logo_s.png"]];
    ...
    [registerButton setBackgroundImage:[UIImage imageNamed:@"Usequence.bundle/Usequence_login_button.png"] forState:UIControlStateNormal];
...
- (void)normalPressed
{
    ...
    {
        [[UsequenceSdkImp sharedInstance]showUsequenceToast:@"用户名为空"];
        return;
    }
    ...
    {
        [[UsequenceSdkImp sharedInstance]showUsequenceToast:@"用户名中不能有空格"];
        return;
    }
    else
    {
    ...
    if (ret == YES) {
        ...
        [[UsequenceSdkImp sharedInstance]showUsequenceToast:@"账户不能为纯数字"];
    ...
    if ([username length] <6 || [username length] > 20) {
          ...
          [[UsequenceSdkImp sharedInstance]showUsequenceToast:@"账户长度为6到20位"];
            return;
    }
    ...
    if (![password isEqualToString:self.secondPassword.text]) {
        [[UsequenceSdkImp sharedInstance]showUsequenceToast:@"对不起你两次输入的密码不匹配"];
        return;
    }
    ...
    if(![pred evaluateWithObject: self.username.text])
    {
        ...
        [[UsequenceSdkImp sharedInstance] showUsequenceToast: @"用户名只能由字母+数字组成,并且字母在数字前面"];
        return;
    }
    ...
    Usequence_VSD_BLOCK successCallback = ^(NSDictionary *dictionary, NSDictionary *headers) {
        ...
        @catch(NSException * e)
        {
            [[UsequenceSdkImp sharedInstance]showUsequenceToast:@"内部错误,请重新注册"];
            self.registering = false;
        }
        @finally
        {
    ...
    [[UsequenceWebInterface sharedInstance] normalUsequenceRegister:username
...
-(void)process {
    if (self.myProcess == nil) {
        self.myProcess = [[UsequenceProgressHUD alloc] initWithView:self.alertContainer];
    }
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    if (textField == self.username){
        ...
        if ([username length] == 0)
        {
            ...
            [[UsequenceSdkImp sharedInstance]showUsequenceToast:@"账户为空"];
        ...
        {
            [[UsequenceSdkImp sharedInstance]showUsequenceToast:@"重复密码为空"];
