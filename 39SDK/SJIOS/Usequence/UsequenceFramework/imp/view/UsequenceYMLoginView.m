...
@interface UsequenceYMLoginView()
...
@property(nonatomic,strong) UsequenceBaseView *baseView;
@property(nonatomic,strong) UsequenceYMLoginView *ymLoginView;//登录界面
...
@implementation UsequenceYMLoginView
...
- (void)judgeUserName
{
    ...
    {
        [[UsequenceSdkImp sharedInstance]showUsequenceToast:@"用户名不能为空"];
        return;
    }
...
-(void)ymStartLogin{
        ...
        @try{
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if(self.textIsphoneNumber){
                    ...
                    [[UsequenceSdkImp sharedInstance] addUsequenceUser:username passwordUsequence:password bindUsequence:@"0" touristUsequence:@"0" phoneNumberUsequence:@"0"];
                }
    ...
    if(self.textIsphoneNumber){//手机号登录
        ...
        [[UsequenceWebInterface sharedInstance] phoneUsequenceLogin:username
                                   ...
                                   successCallbackUsequence:successCallback1 failCallbackUsequence:^(NSError * error) {
                                       self.logining = false;
                                       self.failed(-109);
                                       [[UsequenceSdkImp sharedInstance]showUsequenceToast:@"请检查网络连接！"];
                                   }];
        ...
        [[UsequenceWebInterface sharedInstance] loginUsequence:username
...
- (void) startLogin
{
        ...
        @try{
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if(self.isRegister == YES && [[UsequenceSdkImp sharedInstance] getUsequenceShiMingRenZhengFlag] == YES){
                    [[UsequenceSdkImp sharedInstance] showUsequenceShiMingRenZhengView:self.token callbackUsequence:^(NSString *callbackxxxx){
                        NSString * userid = [dictionary valueForKey:@"id"];
                        ...
                        if ([self.loginInfo.tourist isEqualToString:@"1"]&&[self.loginInfo.bind isEqualToString:@"0"]) {
                            ...
                            [[UsequenceSdkImp sharedInstance] showUsequenceToast:@"该帐号为快速登录帐号,请尽快绑定,以免帐号丢失!"];
                        }
    ...
    //普通注册后的普通登录
        [[UsequenceWebInterface sharedInstance] loginUsequence:self.loginInfo.username
                                     passwordUsequence:[UsequenceSdkMd5 md5:self.loginInfo.password]
                              successCallbackUsequence:successCallback
                                  failCallbackUsequence:^(NSError * error) {
                                      self.login = NO;
                                      self.failed(-106);
