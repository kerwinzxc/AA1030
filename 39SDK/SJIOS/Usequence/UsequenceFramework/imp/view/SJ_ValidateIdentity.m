...
-(void)setLandScapeView{
    ...
    [close setBackgroundImage:[UIImage imageNamed:@"Usequence.bundle/Usequence_ym_close.png"] forState:UIControlStateNormal];
    [close addTarget:self action:@selector(closeUsequencePressed) forControlEvents:UIControlEventTouchUpInside];
    ...
    self.name.background = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Usequence.bundle/Usequence_register_edit_background.png"]];
    ...
    self.identity.background = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Usequence.bundle/Usequence_register_edit_background.png"]];
...
-(void) comfirmPressed{
    ...
    if([self.identity.text length] != 18){
        [[UsequenceSdkImp sharedInstance]showUsequenceToast:@"身份证号码输入有误"];
        return;
    }
        ...
        @try
        {
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if (ok != nil)
                {
                    ...
                    [[UsequenceSdkImp sharedInstance] showUsequenceToast:@"认证成功"];
                    self.complete(@"ok", @"1");
                    [self dismissWithAnimation:YES];
                ...
                {
                    [[UsequenceSdkImp sharedInstance] showUsequenceToast:@"信息有误，认证失败"];
    ...
    [[UsequenceWebInterface sharedInstance] startUsequenceIdentify:self.accesstoken nameUsequence:[self.name.text stringByReplacingOccurrencesOfString:@" " withString:@""] identifyNumberUsequence:[self.identity.text stringByReplacingOccurrencesOfString:@" " withString:@""] successCallbackUsequence:successCallback failCallbackUsequence:^(NSError *error) {
