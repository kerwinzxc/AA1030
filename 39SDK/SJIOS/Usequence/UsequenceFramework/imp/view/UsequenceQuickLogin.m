...
#import "UsequenceYMPhoneRegisterView.h"//跳转到手机注册
#import "UsequenceYMLoginView.h"//跳转到登录界面
#import "UsequenceYMBindPhoneView.h"//跳转到手机绑定
...
@interface UsequenceQuickLogin() <UIAlertViewDelegate>
...
@property(nonatomic,strong) UsequenceYMPhoneRegisterView *ymPhoneRegister;//跳转到手机注册

@property (strong, nonatomic) UsequenceYMBindPhoneView * ymBindPhoneView;//跳转到手机绑定
...
@implementation UsequenceQuickLogin
...
- (id)initWithBlock:(Usequence_SUCCESS_Callback)success failed:(Usequence_FAILED_Callback)failed landScape:(Boolean)landcape {
    ...
    if (self) {
        ...
        if(self.landcape){//横屏状态下显示
            ...
            [quickLogin setBackgroundImage:[UIImage imageNamed:@"Usequence.bundle/Usequence_quick_register.png"] forState:UIControlStateNormal];
            ...
            [cellphoneRegistButton setBackgroundImage:[UIImage imageNamed:@"Usequence.bundle/Usequence_phone_register.png"] forState:UIControlStateNormal];
            ...
            [ymLoginButton setBackgroundImage:[UIImage imageNamed:@"Usequence.bundle/Usequence_login.png"] forState:UIControlStateNormal];
            ...
            [sdkversion setText:Usequence_SDK_VERSION];
...
//            UIImageView * logoView = [[UIImageView alloc] initWithFrame:CGRectMake(85, 17, 95, 38)];
            ...
            [loginButton setBackgroundImage:[UIImage imageNamed:@"Usequence.bundle/Usequence_login.png"] forState:UIControlStateNormal];
...
-(void) chooseClick{
    if(self.agree == true){
        self.agree = false;
        [self.choose setBackgroundImage:[UIImage imageNamed:@"Usequence.bundle/Usequence_noread.png"] forState:
         UIControlStateNormal];
    }else{
        self.agree = true;
        [self.choose setBackgroundImage:[UIImage imageNamed:@"Usequence.bundle/Usequence_read.png"] forState:
...
-(void) quickQuickClick{
    ...
    NSMutableArray *user=[[[UsequenceDataBase alloc]init] getTestList];
    if (user!=nil&&user.count>0) {
        for (int i=0;i<[user count]; i++) {
        UsequenceUserInfo *userinfo=[user objectAtIndex:i];
        if (userinfo.tourist) {
...
-(void) ym_phoneRegister{
    ...
    if (self.ymPhoneRegister==nil) {
        self.ymPhoneRegister=[[UsequenceYMPhoneRegisterView alloc]initWithBlock:self.success failed:self.failed landScape:self.landscape view:self];
    }
...
-(void) image:(UIImage *)image didFinishSavingResult:(NSError *)error contextInfo:(void *)contentInfo
{
    if(error == nil){
      ...
      //  [[UsequenceSdkImp sharedInstance] showUsequenceToast:@"帐号为快速登录帐号,请尽快绑定手机号,以免帐号丢失!\n帐号信息截图已保存到相册中，请妥善保管！"];
...
-(void)fastRegister{
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if ([type isEqualToString:@"1"]) {//type=1没绑定手机 type=0已绑定手机
                    ...
                    if([is_register isEqualToString:@"1"]){
                        ...
                        [[UsequenceSdkImp sharedInstance] sjUsequenceRegister:[dictionary valueForKey:@"id"]];
                 ...
                 UsequenceUserInfo *userinfo=[[UsequenceUserInfo alloc]initWithInfo:user_id password:password bind:bind tourist:@"1" loginTime:[[UsequenceSdkImp sharedInstance]getUsequenceTimeUserNow] phoneNumber:@"0"];
                [self startLogin:userinfo];
            }
            else
            {
    ...
    [[UsequenceWebInterface sharedInstance]fastUsequenceRegisterUsequence:successCallback
...
- (void) startLogin:(UsequenceUserInfo *) userinfo
{
    NSString *password;
    if ([userinfo.tourist  isEqual:@"1"]) {
        password=userinfo.password;
    }else{
        password =[UsequenceSdkMd5 md5:userinfo.password];
    }
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if(self.isRegister == YES && [[UsequenceSdkImp sharedInstance] getUsequenceShiMingRenZhengFlag] == YES){
                    [[UsequenceSdkImp sharedInstance] showUsequenceShiMingRenZhengView:self.token callbackUsequence:^(NSString *callbackssss){
                        ...
                        [[UsequenceSdkImp sharedInstance] addUsequenceUser:userinfo.username passwordUsequence:userinfo.password bindUsequence:userinfo.bind touristUsequence:userinfo.tourist phoneNumberUsequence:userinfo.phoneNumber];
    ...
    [[UsequenceWebInterface sharedInstance] loginUsequence:userinfo.username
