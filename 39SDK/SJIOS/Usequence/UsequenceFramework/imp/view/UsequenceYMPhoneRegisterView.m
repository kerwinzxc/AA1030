...
@property(nonatomic,strong)  UsequenceYMPhoneRegister *ymPhoneRegister;//注册
@property(nonatomic,strong)  UsequenceYMPhoneLogin *ymPhoneLogin;//注册
@property(nonatomic,strong) UsequenceYMPhoneRegisterView *ymPhoneView;//登录界面
...
@implementation UsequenceYMPhoneRegisterView
...
-(void)setLandScapeView{
    ...
    UIImage *edit_up_image = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Usequence.bundle/Usequence_register_edit_background.png"]];
...
-(void)ym_phoneNextButton
{
        ...
        @try
        {
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if (ok != nil)
                {
                    ...
                    [[UsequenceSdkImp sharedInstance]showUsequenceToast:@"此号码未注册过，请注册登录"];
                    
                    
                         self.ymPhoneRegister = [[UsequenceYMPhoneRegister alloc]initWithBlock:^(NSString * username, NSString * password){
                                    UsequenceUserInfo *userinfo=[[UsequenceUserInfo alloc]initWithInfo:username password:password bind:@"1" tourist:@"0" loginTime:0 phoneNumber:@"1"];
                ...
                [[UsequenceSdkImp sharedInstance]showUsequenceToast:@"此号码已经注册过，请输入密码直接登录"];
     
                if (self.ymPhoneLogin==nil) {
                    self.ymPhoneLogin=[[UsequenceYMPhoneLogin alloc]initWithBlock:self.success failed:self.failed landScape:self.landscape view:self.quidklogin ymPhoneNumber:self.ymPhoneNum.text];
    ...
    [[UsequenceWebInterface sharedInstance] checkUsequencePhoneRegist:self.ymPhoneNum.text
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    if(textField==self.username)
    {
        ...
        if ([username length] == 0)
        {
            ...
            [[UsequenceSdkImp sharedInstance]showUsequenceToast:@"请输入正确的手机号"];
...
- (void) startLogin
{
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if(self.isRegister == YES && [[UsequenceSdkImp sharedInstance] getUsequenceShiMingRenZhengFlag] == YES){
                    [[UsequenceSdkImp sharedInstance] showUsequenceShiMingRenZhengView:self.token callbackUsequence:^(NSString *callbackssss){
                        ...
                        [[UsequenceSdkImp sharedInstance] addUsequenceUser:self.loginInfo.username passwordUsequence:self.loginInfo.password bindUsequence:@"1" touristUsequence:@"0" phoneNumberUsequence:@"1"];
    ...
    Usequence_VSD_BLOCK successCallback = ^(NSDictionary *dictionary, NSDictionary *headers) {
        ...
        [[UsequenceWebInterface sharedInstance] phoneUsequenceLogin:self.ymPhoneNum.text
                                          passwordUsequence:self.loginInfo.password
