...
@interface UsequenceDataBase()
@property(strong,nonatomic)UsequenceUserInfo *userinfo;

//0:Restoring.db 1:ggod.db 2:FgcqSJQG.db
@property(nonatomic) int sdkType;

@end
@implementation UsequenceDataBase
...
- (BOOL)openDB {
    ...
    if (find) {
        ...
        //创建一个新表
        [self createUsequenceList:self._database];
...
//创建表
- (BOOL) createUsequenceList:(sqlite3*)db {
    //0:Restoring.db 1:ggod.db 2:FgcqSJQG.db
    char *sql;
    switch (self.sdkType) {
        case 0:
...
//插入数据
-(BOOL) insertUsequenceList:(UsequenceUserInfo *)insertList {
       ...
       // char *sql = "SELECT *FROM Restoringuserinfo";//从testTable这个表中获取 testID, testValue ,testName，若获取全部的话可以用*代替testID, testValue ,testName。
        ...
        if (sqlite3_prepare_v2(_database, sql, -1, &statement, NULL) != SQLITE_OK) {
            ...
            while (sqlite3_step(statement) == SQLITE_ROW) {
                UsequenceUserInfo* sqlList = [[UsequenceUserInfo alloc] init] ;
...
-(BOOL)updateUsequenceUserInfo:(UsequenceUserInfo*)updateUsequenceUserInfo{
    if ([self openDB]) {
       ...
       // char *sql = "update Restoringuserinfo set  logintime = ? , password = ? WHERE username = ?";
        ...
        //绑定text类型的数据库数据
        sqlite3_bind_text(statement, 1, [updateUsequenceUserInfo.loginTime UTF8String], -1, SQLITE_TRANSIENT);
        sqlite3_bind_text(statement, 2, [updateUsequenceUserInfo.password UTF8String], -1, SQLITE_TRANSIENT);
        sqlite3_bind_text(statement, 3, [updateUsequenceUserInfo.username UTF8String], -1, SQLITE_TRANSIENT);
...
//更新数据
-(BOOL) updateUsequenceTourist:(UsequenceUserInfo *)updateList{
...
//删除数据
- (BOOL) deleteUsequenceList:(NSString *)deletList{
    if ([self openDB]) {
        sqlite3_stmt *statement;
        //组织SQL语句
        //0:Restoring.db 1:ggod.db 2:FgcqSJQG.db
...
//查询数据
- (BOOL)searchUsequenceUser:(NSString*)searchString{
