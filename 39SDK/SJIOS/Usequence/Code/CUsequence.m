...
@interface CUsequence ()

@end

@implementation CUsequence
...
- (instancetype)init {
    ...
    for (NSString *countryCode in countryCodes) {
        ...
        CUsequence *metaData = [[CUsequence alloc]init];
        NSLog(@"%@", metaData);
        if (metaData) {
...
- (CUsequence *)getMetadataForRegion:(NSString *)regionCode {
     return nil;
}
...
- (NSString *)inputDigitHelper_:(NSString *)nextChar {
    ...
    if ([NB_REGION_CODE_FOR_NON_GEO_ENTITY isEqualToString:numberWithoutCountryCallingCode]) {
        CUsequence *helper = [[CUsequence alloc] init];
