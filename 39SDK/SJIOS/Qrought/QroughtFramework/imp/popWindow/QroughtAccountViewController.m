...
#import "QroughtAccountViewController.h"
#import "QroughtBindIdentifyViewController.h"
#import "QroughtBindPhoneViewController.h"
#import "QroughtChangePasswordViewController.h"
...
#import "QroughtSdkImp.h"
#import "QroughtBindEmailViewController.h"
@interface QroughtAccountViewController ()

@property (strong, nonatomic) QroughtBindEmailViewController * QroughtBindEmailViewController;
@property (strong, nonatomic) QroughtBindPhoneViewController * QroughtBindPhoneViewController;
@property (strong, nonatomic) QroughtBindIdentifyViewController * QroughtBindIdentifyViewController;
@property (strong, nonatomic) QroughtNavViewController *bindPhoneNavViewController;
@property (strong, nonatomic) QroughtNavViewController *bindIdentifyNavViewController;
@property (strong, nonatomic) QroughtChangePasswordViewController *changePasswordController;
@property (strong, nonatomic) QroughtNavViewController *changePasswordNavController;
@property (strong, nonatomic) QroughtNavViewController  *bindEmailNavController;
@property (strong, nonatomic) QroughtProgressHUD *process;
...
@implementation QroughtAccountViewController
...
-(id)initWithLandscape:(Boolean)landscape
{
    ...
    if (self) {
        ...
        QroughtUserInfo *userinfo=[data objectAtIndex:0];
        ...
        [changePasswordImage setImage:[UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Qrought.bundle/Qrought_ac_menu_change.png"]]];
       ...
       UIImageView *navImageView1 = [[UIImageView alloc]initWithFrame:CGRectMake(changePasswordContainer.frame.size.width - 30, 10, 20, 20)];
        navImageView1.image = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Qrought.bundle/Qrought_ac_down.png"]];
	...
	//绑定手机
        ...
        [bindImage setImage:[UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Qrought.bundle/Qrought_ac_menu_bang.png"]]];
        ...
        navImageView2.image = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Qrought.bundle/Qrought_ac_down.png"]];
        ...
        [emailImage setImage:[UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Qrought.bundle/Qrought_ac_menu_email.png"]]];
        ...
        nav1ImageView2.image = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Qrought.bundle/Qrought_ac_down.png"]];
        ...
        [identifyImage setImage:[UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Qrought.bundle/Qrought_ac_menu_shiming.png"]]];
        ...
        nav2ImageView2.image = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Qrought.bundle/Qrought_ac_down.png"]];
...
-(void)loadingData{
    ...
    [[QroughtWebInterface sharedInstance] checkQroughtPhone:self.accessToken successCallbackQrought:successCallback failCallbackQrought:^(NSError *code) {
        [self showQroughtToast:@"请重新登录帐号"];
    }];
    ...
    [[QroughtWebInterface sharedInstance] checkQroughtMail:self.accessToken successCallbackQrought:successCallback1 failCallbackQrought:^(NSError *code) {
        [self showQroughtToast:@"请重新登录帐号"];
        
    }];
    
    
    Qrought_VSD_BLOCK successCallback2 = ^(NSDictionary *dictionary, NSDictionary *headers) {
        @try
        {
    ...
    [[QroughtWebInterface sharedInstance] checkQroughtIdentify:self.accessToken successCallbackQrought:successCallback2 failCallbackQrought:^(NSError *code) {
    ...
    Qrought_VSD_BLOCK successCallback = ^(NSDictionary *dictionary, NSDictionary *headers) {
        @try
        {
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if ([ok isEqualToString:@"1"])
                {
                    self.process = [[QroughtProgressHUD alloc] initWithView:self.view];
                    [self.view addSubview:self.process];
                    [self.view bringSubviewToFront:self.process];
                    
                    self.process.labelText = @"已经绑定过手机";
                    self.process.mode = QroughtProgressHUDModeCustomView;
                ...
                {
                    if (self.QroughtBindPhoneViewController == nil) {
                        self.QroughtBindPhoneViewController = [[QroughtBindPhoneViewController alloc]initWithLandscape:self.landscape];
                        self.bindPhoneNavViewController = [[QroughtNavViewController alloc]initWithRootViewController:self.QroughtBindPhoneViewController];
                        
                    }
                    self.QroughtBindPhoneViewController.accessToken = self.accessToken;
            ...
            else
            {
                ...
                [self.view addSubview:self.process];
                [self.view bringSubviewToFront:self.process];
                self.process.labelText = [[QroughtSdkImp sharedInstance] translateQrought:[dictionary valueForKey:@"error"]];
                [self.process showAnimated:YES whileExecutingBlock:^{
...
-(void)goToEmail
{
    ...
    Qrought_VSD_BLOCK successCallback = ^(NSDictionary *dictionary, NSDictionary *headers) {
        @try
        {
            ...
            if ((NSNull *)value == nil)
            {
                ...
                {
                    if (self.QroughtBindEmailViewController == nil) {
                        self.QroughtBindEmailViewController = [[QroughtBindEmailViewController alloc]initWithLandscape:self.landscape];
                        self.bindEmailNavController = [[QroughtNavViewController alloc]initWithRootViewController:self.QroughtBindEmailViewController];
                        
                    }
                    self.QroughtBindEmailViewController.accessToken = self.accessToken;
    ...
    [[QroughtWebInterface sharedInstance] checkQroughtMail:self.accessToken successCallbackQrought:successCallback failCallbackQrought:^(NSError *code) {
...
-(void)goToIdentify{
    Qrought_VSD_BLOCK successCallback = ^(NSDictionary *dictionary, NSDictionary *headers) {
        @try
        {
            ...
            if ((NSNull *)value == nil)
            {
                ...
                {
                    if (self.QroughtBindIdentifyViewController == nil) {
                        self.QroughtBindIdentifyViewController = [[QroughtBindIdentifyViewController alloc]initWithLandscape:self.landscape];
                        self.bindIdentifyNavViewController = [[QroughtNavViewController alloc]initWithRootViewController:self.QroughtBindIdentifyViewController];
                        
                    }
                    self.QroughtBindIdentifyViewController.accessToken = self.accessToken;
    ...
    [[QroughtWebInterface sharedInstance] checkQroughtIdentify:self.accessToken successCallbackQrought:successCallback failCallbackQrought:^(NSError *code) {
...
-(void)showQroughtToast:(NSString *)msg{
    if (self.process == nil) {
        self.process = [[QroughtProgressHUD alloc] initWithView:self.view];
    }
...
-(void)goToChangePassword {
    if (self.changePasswordController == nil) {
        self.changePasswordController = [[QroughtChangePasswordViewController alloc]initWithLandscape:self.landscape];
        self.changePasswordNavController = [[QroughtNavViewController alloc]initWithRootViewController:self.changePasswordController];
    }
