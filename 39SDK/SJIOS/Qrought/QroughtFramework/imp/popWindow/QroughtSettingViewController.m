...
#import "QroughtServiceViewController.h"
#import "QroughtFloatViewController.h"
#import "QroughtSdkImp.h"
@interface QroughtSettingViewController ()

@property (strong, nonatomic) QroughtServiceViewController *QroughtServiceViewController;
@property (strong, nonatomic) QroughtAccountViewController *accountController;
@property (strong, nonatomic) QroughtAccountViewController *accountController2;

@property (strong, nonatomic) UIView *container;
@property (strong, nonatomic) UIView *serviceContainer;
@property (strong, nonatomic) UIView *accountContainer;
@property (strong, nonatomic) UIView *hideFloatQroughtContainer;
...
@implementation QroughtSettingViewController
...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    self.QroughtServiceViewController = [[QroughtServiceViewController alloc]initWithLandscape:self.landscape];
    self.QroughtServiceViewController.pageIndex = 0;
    self.QroughtServiceViewController.view.autoresizingMask = UIViewAutoresizingFlexibleTopMargin;
    
    self.accountController = [[QroughtAccountViewController alloc]initWithLandscape:self.landscape];
    ...
    QroughtFloatViewController *targetController;
    if (self.pageIndex == 0) {
        targetController = self.QroughtServiceViewController;
...
-(void)initNavigation {
    UIImage *backNormalImage = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Qrought.bundle/Qrought_back.png"]] ;
    UIImage *backHighImage = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Qrought.bundle/Qrought_back_cl.png"]] ;
...
//    UIImage *backgroundImage = [UIImage imageWithColor:[UIColor colorWithRed:0.1176 green:0.5255 blue:0.855 alpha:1.0] cornerRadius:3.0f sizeOfdialog:CGSizeMake(screenSize.width, 44)];
 ...
 //   [self.navigationController.navigationBar setBackgroundImage:backgroundImage forBarMetrics:UIBarMetricsDefault];
   // self.navigationController.navigationBar.tintColor=[UIColor colorWithRed:0.1176 green:0.5255 blue:0.855 alpha:1.0];
    ...
    serviceImageView.image = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Qrought.bundle/Qrought_icon_meun_ser.png"]] ;
    ...
    accountImageView.image = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Qrought.bundle/Qrought_icon_meun_ac.png"]] ;
...
-(void)serviceTap {
    ...
    [self.serviceContainer addSubview:self.blueBar];
    self.pageIndex = 0;
    [self setViewControllers:@[self.QroughtServiceViewController]
                   direction:UIPageViewControllerNavigationDirectionForward
                    animated:NO
                  completion:NULL];
}
...
- (UIViewController *)pageViewController:(UIPageViewController *)pvc viewControllerBeforeViewController:(QroughtFloatViewController *)vc
{
    if (vc.pageIndex == 1) {
        self.QroughtServiceViewController.delegate = self;
        self.pageIndex = 0;
        return self.QroughtServiceViewController;
    }
    return nil;
}

- (UIViewController *)pageViewController:(UIPageViewController *)pvc viewControllerAfterViewController:(QroughtFloatViewController *)vc
{
    if (vc.pageIndex == 0) {
