...
@property (nonatomic,strong)  QroughtQuickLogin *quidklogin;
@property (nonatomic,strong) QroughtDefaultLogin *defaultLogin;
...
@property(nonatomic,strong)  QroughtRegisterView *registerView;//注册
@property(nonatomic,strong) QroughtForgetView *forgetPhoneView;//手机找回密码界面
@property(nonatomic) Boolean SHOWPROCESS;
@property(nonatomic,strong) QroughtBaseView *baseView;
@property(nonatomic,strong) QroughtYMLoginView *ymLoginView;//登录界面
...
@implementation QroughtYMLoginView
...
-(void)setLandScapeView{
    ...
    UIImage * logo = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Qrought.bundle/Qrought_logo.png"]];
    ...
    UIImage *edit_up_image = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Qrought.bundle/Qrought_register_edit_background.png"]];
    ...
    UIImage * logo_r = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Qrought.bundle/Qrought_logo_r.png"]];
    ...
    password.background = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Qrought.bundle/Qrought_register_edit_background.png"]];
    ...
    UIImage * logo_s = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Qrought.bundle/Qrought_logo_s.png"]];
    ...
    UIImage *ymLoginBackImage = [[UIImage alloc]initWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Qrought.bundle/Qrought_login_button.png"]];
...
- (void)judgeUserName
{
    ...
    if([password length]== 0){
        [[QroughtSdkImp sharedInstance]showQroughtToast:@"请输入密码"];
        return;
    }
    ...
    {
        [[QroughtSdkImp sharedInstance]showQroughtToast:@"用户名不能为空"];
        return;
    }

    if ([password length] == 0)
    {
        [[QroughtSdkImp sharedInstance]showQroughtToast:@"密码不能为空"];
...
-(void)ymStartLogin{
    ...
    Qrought_VSD_BLOCK getUserIdCompletionBlock = ^(NSDictionary *dictionary, NSDictionary *headers){
        @try{
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if(self.textIsphoneNumber){
                    [[QroughtSdkImp sharedInstance] addQroughtUser:username passwordQrought:password bindQrought:@"1" touristQrought:@"0" phoneNumberQrought:@"1"];
                }else{
                    [[QroughtSdkImp sharedInstance] addQroughtUser:username passwordQrought:password bindQrought:@"0" touristQrought:@"0" phoneNumberQrought:@"0"];
                }
        ...
        {
            [[QroughtSdkImp sharedInstance]showQroughtToast:@"内部错误" ];
            self.logining = false;
        }
        @finally
        {
    ...
    if(self.textIsphoneNumber){//手机号登录
        ...
        [[QroughtWebInterface sharedInstance] phoneQroughtLogin:username
                                          passwordQrought:password
                                   successCallbackQrought:successCallback1 failCallbackQrought:^(NSError * error) {
                                       self.logining = false;
                                       self.failed(-109);
                                       [[QroughtSdkImp sharedInstance]showQroughtToast:@"请检查网络连接！"];
                                   }];
        ...
        [[QroughtWebInterface sharedInstance] loginQrought:username
...
- (void) startLogin
{
        ...
        @try{
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if(self.isRegister == YES && [[QroughtSdkImp sharedInstance] getQroughtShiMingRenZhengFlag] == YES){
                    [[QroughtSdkImp sharedInstance] showQroughtShiMingRenZhengView:self.token callbackQrought:^(NSString *callbackxxxx){
                        NSString * userid = [dictionary valueForKey:@"id"];
                        ...
                        if ([self.loginInfo.tourist isEqualToString:@"1"]&&[self.loginInfo.bind isEqualToString:@"0"]) {
                            ...
                            [[QroughtSdkImp sharedInstance] showQroughtToast:@"该帐号为快速登录帐号,请尽快绑定,以免帐号丢失!"];
                        }
    ...
    //普通注册后的普通登录
        [[QroughtWebInterface sharedInstance] loginQrought:self.loginInfo.username
                                     passwordQrought:[QroughtSdkMd5 md5:self.loginInfo.password]
                              successCallbackQrought:successCallback
                                  failCallbackQrought:^(NSError * error) {
                                      self.login = NO;
                                      self.failed(-106);
...
-(void)ymdismissWithAnimation:(BOOL)animated
{
    ...
    [[QroughtSdkImp sharedInstance] showQroughtWelcome:[self.username text]];
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    ...
    else if(textField==self.password)
    {
        ...
        if ([username length] == 0)
        {
            ...
            [[QroughtSdkImp sharedInstance]showQroughtToast:@"用户名为空"];
