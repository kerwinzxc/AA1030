...
@property(nonatomic,strong) QroughtBind *ymBind;//绑定正式帐号界面

@end


@implementation QroughtBind
...
-(void) image:(UIImage *)image didFinishSavingResult:(NSError *)error contextInfo:(void *)contentInfo
{
    if(error == nil){
        ...
        [[QroughtSdkImp sharedInstance] showQroughtToast:@"绑定成功！帐号信息截图已保存到相册中，帐号丢失可以查看截图找回，请妥善保管！"];
...
- (void)bindPressed
{
    ...
    if(![pred evaluateWithObject: self.username.text])
    {
        ...
        [[QroughtSdkImp sharedInstance] showQroughtToast: @"用户名只能由字母跟数字组成"];
        return;
    }
    ...
    [[QroughtWebInterface sharedInstance] bindQroughtRegister:username
                                          passwordQrought:[QroughtSdkMd5 md5:password]
                                    bindUsernameQrought:self.bindname
