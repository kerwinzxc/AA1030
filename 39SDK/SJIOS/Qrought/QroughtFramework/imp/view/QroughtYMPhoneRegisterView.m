...
#import "QroughtYMPhoneRegisterView.h"
...
#import "QroughtYMPhoneRegister.h"
#import "QroughtYMPhoneLogin.h"
#define Qrought_USER_AGREEMENT_URL @"https://www.3975.com/public/web/39XY.html"

@interface QroughtYMPhoneRegisterView()
...
@property(nonatomic,strong)  QroughtYMPhoneRegister *ymPhoneRegister;//注册
@property(nonatomic,strong)  QroughtYMPhoneLogin *ymPhoneLogin;//注册
@property(nonatomic,strong) QroughtYMPhoneRegisterView *ymPhoneView;//登录界面
...
@implementation QroughtYMPhoneRegisterView
...
-(void)setPortraitView{
    ...
    ymloginButton.zhw_ignoreEvent = NO;//yes不启用防止重复点击，NO启用防止重复点击
    ymloginButton.zhw_acceptEventInterval = 3.0;//重复点击间隔时间
    [ymloginButton setBackgroundImage:[UIImage imageNamed:@"Qrought.bundle/Qrought_login_button.png"] forState:UIControlStateNormal];
    [ymloginButton addTarget:self action:@selector(ym_phoneNextButton) forControlEvents:UIControlEventTouchUpInside];
    [self.alertContainer addSubview:ymloginButton];
}
...
-(void)ym_phoneNextButton
{
        ...
        @try
        {
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if (ok != nil)
                {
                    ...
                    [[QroughtSdkImp sharedInstance]showQroughtToast:@"此号码未注册过，请注册登录"];
                    
                    
                         self.ymPhoneRegister = [[QroughtYMPhoneRegister alloc]initWithBlock:^(NSString * username, NSString * password){
                                    QroughtUserInfo *userinfo=[[QroughtUserInfo alloc]initWithInfo:username password:password bind:@"1" tourist:@"0" loginTime:0 phoneNumber:@"1"];
                ...
                [[QroughtSdkImp sharedInstance]showQroughtToast:@"此号码已经注册过，请输入密码直接登录"];
     
                if (self.ymPhoneLogin==nil) {
                    self.ymPhoneLogin=[[QroughtYMPhoneLogin alloc]initWithBlock:self.success failed:self.failed landScape:self.landscape view:self.quidklogin ymPhoneNumber:self.ymPhoneNum.text];
    ...
    [[QroughtWebInterface sharedInstance] checkQroughtPhoneRegist:self.ymPhoneNum.text
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    if(textField==self.username)
    {
        ...
        if ([username length] == 0)
        {
            ...
            [[QroughtSdkImp sharedInstance]showQroughtToast:@"请输入正确的手机号"];
...
- (void) startLogin
{
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if(self.isRegister == YES && [[QroughtSdkImp sharedInstance] getQroughtShiMingRenZhengFlag] == YES){
                    [[QroughtSdkImp sharedInstance] showQroughtShiMingRenZhengView:self.token callbackQrought:^(NSString *callbackssss){
                        ...
                        [[QroughtSdkImp sharedInstance] addQroughtUser:self.loginInfo.username passwordQrought:self.loginInfo.password bindQrought:@"1" touristQrought:@"0" phoneNumberQrought:@"1"];
    ...
    Qrought_VSD_BLOCK successCallback = ^(NSDictionary *dictionary, NSDictionary *headers) {
        ...
        [[QroughtWebInterface sharedInstance] phoneQroughtLogin:self.ymPhoneNum.text
                                          passwordQrought:self.loginInfo.password
                                   successCallbackQrought:successCallback failCallbackQrought:^(NSError * error) {
            self.login = NO;
            self.failed(-106);
            [[QroughtSdkImp sharedInstance]showQroughtToast:@"请检查网络连接！"];
        }];
    ...
    [self processQroughtShowLogin];
}

-(void)processQroughtShowLogin {
    if (self.process == nil) {
        self.process = [[QroughtProgressHUD alloc] initWithView:self.alertContainer];
