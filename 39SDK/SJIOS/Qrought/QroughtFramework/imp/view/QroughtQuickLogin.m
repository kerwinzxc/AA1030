...
#import "QroughtYMPhoneRegisterView.h"//跳转到手机注册
#import "QroughtYMLoginView.h"//跳转到登录界面
#import "QroughtYMBindPhoneView.h"//跳转到手机绑定
...
@interface QroughtQuickLogin() <UIAlertViewDelegate>
...
@property(nonatomic,strong) QroughtYMPhoneRegisterView *ymPhoneRegister;//跳转到手机注册

@property (strong, nonatomic) QroughtYMBindPhoneView * ymBindPhoneView;//跳转到手机绑定
...
@implementation QroughtQuickLogin
...
- (id)initWithBlock:(Qrought_SUCCESS_Callback)success failed:(Qrought_FAILED_Callback)failed landScape:(Boolean)landcape {
    ...
    if (self) {
        ...
        if(self.landcape){//横屏状态下显示
            ...
            [quickLogin setBackgroundImage:[UIImage imageNamed:@"Qrought.bundle/Qrought_quick_register.png"] forState:UIControlStateNormal];
            ...
            [cellphoneRegistButton setBackgroundImage:[UIImage imageNamed:@"Qrought.bundle/Qrought_phone_register.png"] forState:UIControlStateNormal];
            ...
            [ymLoginButton setBackgroundImage:[UIImage imageNamed:@"Qrought.bundle/Qrought_login.png"] forState:UIControlStateNormal];
            ...
            [sdkversion setText:Qrought_SDK_VERSION];
            ...
            if(IS_SHOW_AGREE){
                ...
                [self.choose setBackgroundImage:[UIImage imageNamed:@"Qrought.bundle/Qrought_read.png"] forState:
...
//            UIImageView * logoView = [[UIImageView alloc] initWithFrame:CGRectMake(85, 17, 95, 38)];
            ...
            [loginButton setBackgroundImage:[UIImage imageNamed:@"Qrought.bundle/Qrought_login.png"] forState:UIControlStateNormal];
...
-(void) chooseClick{
    if(self.agree == true){
        self.agree = false;
        [self.choose setBackgroundImage:[UIImage imageNamed:@"Qrought.bundle/Qrought_noread.png"] forState:
         UIControlStateNormal];
    }else{
        self.agree = true;
        [self.choose setBackgroundImage:[UIImage imageNamed:@"Qrought.bundle/Qrought_read.png"] forState:
...
-(void) quickQuickClick{
    ...
    NSMutableArray *user=[[[QroughtDataBase alloc]init] getTestList];
    if (user!=nil&&user.count>0) {
        for (int i=0;i<[user count]; i++) {
        QroughtUserInfo *userinfo=[user objectAtIndex:i];
        if (userinfo.tourist) {
...
-(void) ym_phoneRegister{
    ...
    if (self.ymPhoneRegister==nil) {
        self.ymPhoneRegister=[[QroughtYMPhoneRegisterView alloc]initWithBlock:self.success failed:self.failed landScape:self.landscape view:self];
    }
...

{
    ...
    UIImage *image = [ UIImage imageNamed : @"Qrought.bundle/Qrought_buttombackground"];
    UIImage *imagelogo = [ UIImage imageNamed : @"Qrought.bundle/Qrought_Qrought_logo"];
...
-(void) image:(UIImage *)image didFinishSavingResult:(NSError *)error contextInfo:(void *)contentInfo
{
    if(error == nil){
      ...
      //  [[QroughtSdkImp sharedInstance] showQroughtToast:@"帐号为快速登录帐号,请尽快绑定手机号,以免帐号丢失!\n帐号信息截图已保存到相册中，请妥善保管！"];
...
-(void)fastRegister{
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if ([type isEqualToString:@"1"]) {//type=1没绑定手机 type=0已绑定手机
                    ...
                    if([is_register isEqualToString:@"1"]){
                        ...
                        [[QroughtSdkImp sharedInstance] sjQroughtRegister:[dictionary valueForKey:@"id"]];
                 ...
                 QroughtUserInfo *userinfo=[[QroughtUserInfo alloc]initWithInfo:user_id password:password bind:bind tourist:@"1" loginTime:[[QroughtSdkImp sharedInstance]getQroughtTimeUserNow] phoneNumber:@"0"];
                [self startLogin:userinfo];
            }
            else
            {
    ...
    [[QroughtWebInterface sharedInstance]fastQroughtRegisterQrought:successCallback
...
- (void) startLogin:(QroughtUserInfo *) userinfo
{
    NSString *password;
    if ([userinfo.tourist  isEqual:@"1"]) {
        password=userinfo.password;
    }else{
        password =[QroughtSdkMd5 md5:userinfo.password];
    }
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if(self.isRegister == YES && [[QroughtSdkImp sharedInstance] getQroughtShiMingRenZhengFlag] == YES){
                    [[QroughtSdkImp sharedInstance] showQroughtShiMingRenZhengView:self.token callbackQrought:^(NSString *callbackssss){
                        ...
                        [[QroughtSdkImp sharedInstance] addQroughtUser:userinfo.username passwordQrought:userinfo.password bindQrought:userinfo.bind touristQrought:userinfo.tourist phoneNumberQrought:userinfo.phoneNumber];
    ...
    [[QroughtWebInterface sharedInstance] loginQrought:userinfo.username
