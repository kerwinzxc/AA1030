...
@interface QroughtRegisterView()<UIAlertViewDelegate>
...
@property(nonatomic,strong) QroughtRegisterView *ymRegisterView;//帐号注册界面

@end

@implementation QroughtRegisterView
...
-(void)setLandScapeView{
    ...
    username.background = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Qrought.bundle/Qrought_register_edit_background.png"]];
    ...
    secondpassword.background = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Qrought.bundle/Qrought_register_edit_background.png"]];
    ...
    UIImage *logo_p=[UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Qrought.bundle/Qrought_logo_s.png"]];
    ...
    [registerButton setBackgroundImage:[UIImage imageNamed:@"Qrought.bundle/Qrought_login_button.png"] forState:UIControlStateNormal];
...
- (void)normalPressed
{
    ...
    {
        [[QroughtSdkImp sharedInstance]showQroughtToast:@"用户名中不能有空格"];
        return;
    }
    else
    {
    ...
    if (ret == YES) {
        ...
        [[QroughtSdkImp sharedInstance]showQroughtToast:@"账户不能为纯数字"];
    ...
    if ([username length] <6 || [username length] > 20) {
          ...
          [[QroughtSdkImp sharedInstance]showQroughtToast:@"账户长度为6到20位"];
            return;
    }
    ...
    if (![password isEqualToString:self.secondPassword.text]) {
        [[QroughtSdkImp sharedInstance]showQroughtToast:@"对不起你两次输入的密码不匹配"];
        return;
    }
    ...
    if(![pred evaluateWithObject: self.username.text])
    {
        ...
        [[QroughtSdkImp sharedInstance] showQroughtToast: @"用户名只能由字母+数字组成,并且字母在数字前面"];
        return;
    }
    ...
    Qrought_VSD_BLOCK successCallback = ^(NSDictionary *dictionary, NSDictionary *headers) {
        ...
        @catch(NSException * e)
        {
            [[QroughtSdkImp sharedInstance]showQroughtToast:@"内部错误,请重新注册"];
            self.registering = false;
        }
        @finally
        {
    ...
    [[QroughtWebInterface sharedInstance] normalQroughtRegister:username
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    if (textField == self.username){
        ...
        if ([username length] == 0)
        {
            ...
            [[QroughtSdkImp sharedInstance]showQroughtToast:@"账户为空"];
        ...
        {
            [[QroughtSdkImp sharedInstance]showQroughtToast:@"重复密码为空"];
