...
#import "QroughtYMLoginView.h"

#define Qrought_USER_AGREEMENT_URL @"https://www.3975.com/public/web/39XY.html"

@interface QroughtDefaultLogin()
@property (nonatomic, strong) QroughtRegisterView *registerView;
@property (nonatomic,strong) QroughtBind *bindview;
...
@property(nonatomic, strong) Qrought_SUCCESS_Callback success;
@property(nonatomic, strong) Qrought_FAILED_Callback failed;
@property (nonatomic,assign) Boolean landcape;
@property (nonatomic,strong) QroughtUserInfo *loginInfo;
//@property (nonatomic,strong) QroughtUserInfo *phoneLoginInfo;
...
@property(nonatomic,strong) QroughtForgetView *forgetPhoneView;//手机找回密码界面
@property(nonatomic,strong) QroughtYMLoginView *ymLoginView;//跳转到帐号登录页面
@property(nonatomic) NSUInteger usernameRow;//账户列表行数

//用户列表新增==========
@property(nonatomic, strong)QroughtDataBase *QroughtDatabase;//数据库
//初始化数据库，查询用户列表并显示
...
@implementation QroughtDefaultLogin


- (id)initWithBlock:(Qrought_SUCCESS_Callback)success failed:(Qrought_FAILED_Callback)failed landScape:(Boolean)landcape {
    ...
    self.QroughtDatabase = [[QroughtDataBase alloc]init];
    ...
    if (self) {
        ...
        if (self.QroughtDatabase) {
            
            self.alluserinfo=[self.QroughtDatabase getTestList];
        }else{
...
-(void)setLandScapeView{
    ...
    if([_alluserinfo count] > 0){
        ...
        NSArray *data=[[[[QroughtDataBase alloc]init] getTestList] sortedArrayUsingComparator:^NSComparisonResult(QroughtUserInfo *obj1, QroughtUserInfo  *obj2) {
            
            return [obj2.loginTime compare:obj1.loginTime];
        }];
        
        self.dataList=[NSMutableArray arrayWithArray:data];
        QroughtUserInfo *userinfo=[self.dataList objectAtIndex:0];
        self.loginInfo=userinfo;
 }
 
    UIImageView * logoView = [[UIImageView alloc] initWithFrame:CGRectMake(150, 20, 166, 68)];
    logoView.image=[UIImage imageNamed:@"Qrought.bundle/Qrought_logo.png"];
    ...
    [self.defaultButton setBackgroundImage:[UIImage imageNamed:@"Qrought.bundle/Qrought_register_edit_background.png"] forState:UIControlStateNormal];
    ...
    self.arrow.image=[UIImage imageNamed:@"Qrought.bundle/Qrought_arrow.png"];
    ...
    [loginButton setBackgroundImage:[UIImage imageNamed:@"Qrought.bundle/Qrought_login_button.png"] forState:UIControlStateNormal];
    ...
    [_registButton setBackgroundImage:[UIImage imageNamed:@"Qrought.bundle/Qrought_login_button.png"] forState:UIControlStateNormal];
...
-(void)agree2{
    ...
    [[UIApplication sharedApplication] openURL:[NSURL URLWithString:Qrought_USER_AGREEMENT_URL]];
}

-(void) chooseClick{
    if(self.agree == true){
        self.agree = false;
        [self.choose setBackgroundImage:[UIImage imageNamed:@"Qrought.bundle/Qrought_unOK.png"] forState:
         UIControlStateNormal];
    }else{
        self.agree = true;
        [self.choose setBackgroundImage:[UIImage imageNamed:@"Qrought.bundle/Qrought_OK.png"] forState:
         UIControlStateNormal];
    }
}
...
-(void)forgetPhoneButton
{
    ...
    if(self.agree == false){
        [[QroughtSdkImp sharedInstance] showQroughtToast:@"请详细阅读并同意用户协议"];
        return;
    }
    
    if (self.forgetPhoneView == nil) {
        self.forgetPhoneView = [[QroughtForgetView alloc] initWithLandscape:self.landscape];
    }
...
- (void) startLogin
{
    ...
    if([self.loginInfo.password isEqualToString:@"0"]){
        ...
        //if (self.ymLoginView==nil) {
            self.ymLoginView=[[QroughtYMLoginView alloc]initWithBlock:self.success failed:self.failed landScape:self.landscape view:self username:self.loginInfo.username];
        //}
    ...
    if ([self.loginInfo.tourist  isEqual:@"1"]) {
        ...
        password=[QroughtSdkMd5 md5:self.loginInfo.password];
    }

    Qrought_VSD_BLOCK getUserIdCompletionBlock = ^(NSDictionary *dictionary, NSDictionary *headers){
        @try{
            if (dictionary == nil)
            {
                self.login = NO;
                [[QroughtSdkImp sharedInstance]showQroughtToast:@"网络不太好，请重试！！！" ];
                return;
            }
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if(self.isRegister == YES && [[QroughtSdkImp sharedInstance] getQroughtShiMingRenZhengFlag] == YES){
                    [[QroughtSdkImp sharedInstance] showQroughtShiMingRenZhengView:self.token callbackQrought:^(NSString *callbackssss){
                        ...
                        [[QroughtWebInterface sharedInstance] QroughtsendHeart:userid];
                        [[UIApplication sharedApplication].keyWindow endEditing:YES];
                        
                        if ([self.loginInfo.tourist isEqualToString:@"1"]&&[self.loginInfo.bind isEqualToString:@"0"]) {
                            [[QroughtSdkImp sharedInstance] showQroughtToast:@"该帐号为快速登录帐号,请尽快绑定手机号,以免帐号丢失!"];
                        }
                        [[QroughtSdkImp sharedInstance] addQroughtUser:self.loginInfo.username passwordQrought:self.loginInfo.password bindQrought:self.loginInfo.bind    touristQrought:self.loginInfo.tourist phoneNumberQrought:self.loginInfo.phoneNumber];
            ...
            {
                [[QroughtSdkImp sharedInstance]showQroughtToast:[[QroughtSdkImp sharedInstance] translateQrought:[dictionary valueForKey:@"error"]]];
        ...
        {
            //[[QroughtSdkImp sharedInstance]showQroughtToast:@"登录失败，请退出重新进游戏" ];
            self.login = NO;
        }
        @finally
        {
    ...
    Qrought_VSD_BLOCK getTokenCompletionBlock = ^(NSDictionary *dictionary, NSDictionary *headers){
        @try{
            if (dictionary == nil)
            {
                self.login = NO;
            ...
            if ((NSNull *)value == nil)
            {
                ...
                [[QroughtWebInterface sharedInstance] getQroughtUserId:access_token
                                          successCallbackQrought:getUserIdCompletionBlock
        ...
        {
            [[QroughtSdkImp sharedInstance]showQroughtToast:@"获取信息失败，请退出重新进游戏" ];
            self.login = NO;
    ...
    Qrought_VSD_BLOCK successCallback = ^(NSDictionary *dictionary, NSDictionary *headers) {
        @try
        {
            ...
            if ((NSNull *)value == nil)
            {
                ...
                [[QroughtWebInterface sharedInstance] getQroughtAccessTokenQrought:[dictionary valueForKey:@"code"]
                                               successCallbackQrought:getTokenCompletionBlock
                                                   failCallbackQrought:^(NSError *error){
                                                       self.login = NO;
                                                       self.failed(-105);
                                                       [[QroughtSdkImp sharedInstance]showQroughtToast:@"请检查网络连接！"];
        ...
        @catch(NSException * e)
        {
            ...
            [[QroughtSdkImp sharedInstance] showQroughtToast:@"验证失败，请退出重新进游戏"];
            self.login = NO;
    ...
    Qrought_VSD_BLOCK successCallback1 = ^(NSDictionary *dictionary, NSDictionary *headers) {
        // self.login = NO;
        @try
        {
    ...
    if([self.loginInfo.phoneNumber isEqual:@"1"]){
        ...
        [[QroughtWebInterface sharedInstance] phoneQroughtLogin:self.loginInfo.username
                                          passwordQrought:password
                                   successCallbackQrought:successCallback1
                                       failCallbackQrought:^(NSError * error) {
                                       self.login = NO;
                                       self.failed(-106);
                                       [[QroughtSdkImp sharedInstance]showQroughtToast:@"请检查网络连接！"];
    ...
    [[QroughtWebInterface sharedInstance] loginQrought:self.loginInfo.username
                                 passwordQrought:password
...
-(void)checkAccountIdentity:(NSString *)access_token{
    [[QroughtWebInterface sharedInstance] checkQroughtIdentify:access_token successCallbackQrought:^(NSDictionary *dictionary, NSDictionary *headers) {
        @try
        {
            if (dictionary == nil)
            {
    ...
    } failCallbackQrought:^(NSError *code) {
        [self dismissWithAnimation:YES];
        [[QroughtSdkImp sharedInstance]showQroughtToast:@"获取身份证认证信息失败" ];
    }];
}
...
-(void)processQroughtShow {
    if (self.process == nil) {
        self.process = [[QroughtProgressHUD alloc] initWithView:self.alertContainer];
...
- (void) registerPressed
{
    ...
    if ([self.loginInfo.tourist isEqual:@"1"]&&[self.loginInfo.bind isEqual:@"0"]) {
        ...
        if(self.bindview==nil){
            self.bindview=[[QroughtBind alloc]initWithBlock:^(NSString *username, NSString *password) {
                QroughtUserInfo *userinfo=[[QroughtUserInfo alloc]initWithInfo:username password:password bind:@"1" tourist:@"0" loginTime:0 phoneNumber:@"0"];
                self.loginInfo=userinfo;
                [self startLogin];
        ...
        [[QroughtSdkImp sharedInstance] removeQroughtUser:self.loginInfo.username];//绑定后删掉快速登录帐号
        
    }else{
        
        
          if (self.registerView == nil) {
        self.registerView = [[QroughtRegisterView alloc] initWithBlock:^(NSString * username, NSString * password) {
            QroughtUserInfo *userinfo=[[QroughtUserInfo alloc]initWithInfo:username password:password bind:@"0" tourist:@"0" loginTime:0 phoneNumber:@"0"];
            self.loginInfo=userinfo;
            self.isRegister = YES;
            [self startLogin];
...
-(void) footClick{
    ...
    if (self.ymLoginView==nil) {
        self.ymLoginView=[[QroughtYMLoginView alloc]initWithBlock:self.success failed:self.failed landScape:self.landscape view:self];
    }
    [self.ymLoginView show:self];
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    QroughtUserInfo *info=[self.dataList objectAtIndex:row];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    QroughtUserInfo *info=[self.dataList objectAtIndex:[indexPath row]];
    self.loginInfo=info;
...
-(void) tableView:(UITableView *)tableView commitEditingStyle:(UITableViewCellEditingStyle)editingStyle forRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSUInteger row=[indexPath row];
    QroughtUserInfo *info=[self.dataList objectAtIndex:row];
    [[QroughtSdkImp sharedInstance] removeQroughtUser:info.username];
    ...
    if (self.dataList.count>0) {
        ...
        [self dismissWithAnimation:YES];
        [[QroughtSdkImp sharedInstance]loginQrought:[QroughtSdkImp sharedInstance].rootViewQrought landscapeQrought:self.landcape successQrought:self.success failedQrought:self.failed flagQrought:1];
