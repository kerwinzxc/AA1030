...
@interface QroughtYMPhoneRegister()
...
@implementation QroughtYMPhoneRegister
...
-(void)setLandScapeView{
    ...
    UIImage * logo_r = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Qrought.bundle/Qrought_logo_s.png"]];
    ...
    UIImage *verifybackImage = [[UIImage alloc]initWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Qrought.bundle/Qrought_login_button.png"]] ;
...
-(void)ym_verifyPressed:(id)sender {
    ...
    Qrought_VSD_BLOCK successCallback = ^(NSDictionary *dictionary, NSDictionary *headers) {
        ...
        @try
        {
            ...
            if ((NSNull *)value == nil)
            {
                ...
                [[QroughtSdkImp sharedInstance]showQroughtToast:@"获取验证码成功"];
                self.logining = false;
            ...
            {
                self.myProcess = [[QroughtProgressHUD alloc] initWithView:self.alertContainer];
                [self.alertContainer addSubview:self.myProcess];
                [self.alertContainer bringSubviewToFront:self.myProcess];
                self.myProcess.labelText = [[QroughtSdkImp sharedInstance] translateQrought:[dictionary valueForKey:@"error"]];
                self.myProcess.mode = QroughtProgressHUDModeCustomView;
    ...
    [[QroughtWebInterface sharedInstance] phoneQroughtRegistVerify:self.ymPhoneNumber
...
[[UIApplication sharedApplication].keyWindow endEditing:YES];
    ...
    if (verify.length == 0)
    {
        [[QroughtSdkImp sharedInstance]showQroughtToast:@"请输入验证码"];
...
//                [self setRandomAccount];
    ...
    [[QroughtWebInterface sharedInstance] phoneQroughtRegist:self.ymPhoneNumber
                                            passwordQrought:self.password.text
                                          verifyCodeQrought:self.verifyField.text
                                     successCallbackQrought:successCallback
                                         failCallbackQrought:^(NSError * error) {
                                             self.registering = false;
