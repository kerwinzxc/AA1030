...
#import "QroughtYMBindPhoneView.h"
...
@interface QroughtYMBindPhoneView()
...
@property(nonatomic,strong) QroughtYMBindPhoneView *ymBindPhoneView;//忘记密码找回界面
...
@implementation QroughtYMBindPhoneView
...
-(void)setLandScapeView{
    ...
    [closeButton setBackgroundImage:[UIImage imageNamed:@"Qrought.bundle/Qrought_ym_close.png"] forState:UIControlStateNormal];
...
{
    QroughtForgetEmailView *forgetView = [[QroughtForgetEmailView alloc]initWithLandscape:self.landscape];
...
- (void)verifyPressed
{
    ...
    if (phone.length == 0)
    {
        ...
        [[QroughtSdkImp sharedInstance]showQroughtToast:@"请输入手机号"];
        return;
    }
    ...
    if(self.textIsphoneNumber){
        ...
        [[QroughtWebInterface sharedInstance] bindQroughtPhone:self.accessToken
                                            phoneQrought:phone
...
- (void)normalPressed
{
    ...
    if (username.length == 0)
    {
        ...
        [[QroughtSdkImp sharedInstance]showQroughtToast:@"手机号或者密码为空"];
        return;
    }
...
//                    [self checkAccountIdentity:self.accessToken];//检查有没有实名
    ...
    [[QroughtWebInterface sharedInstance] confirmQroughtPhone:self.accessToken
