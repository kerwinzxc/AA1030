...
-(void)setLandScapeView{
    ...
    [close setBackgroundImage:[UIImage imageNamed:@"Qrought.bundle/Qrought_ym_close.png"] forState:UIControlStateNormal];
    [close addTarget:self action:@selector(closeQroughtPressed) forControlEvents:UIControlEventTouchUpInside];
    ...
    self.name.background = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Qrought.bundle/Qrought_register_edit_background.png"]];
    ...
    self.identity.background = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Qrought.bundle/Qrought_register_edit_background.png"]];
...
-(void) comfirmPressed{
    ...
    if([self.identity.text length] != 18){
        [[QroughtSdkImp sharedInstance]showQroughtToast:@"身份证号码输入有误"];
        return;
    }
        ...
        @try
        {
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if (ok != nil)
                {
                    ...
                    [[QroughtSdkImp sharedInstance] showQroughtToast:@"认证成功"];
                    self.complete(@"ok", @"1");
                    [self dismissWithAnimation:YES];
                ...
                {
                    [[QroughtSdkImp sharedInstance] showQroughtToast:@"信息有误，认证失败"];
    ...
    [[QroughtWebInterface sharedInstance] startQroughtIdentify:self.accesstoken nameQrought:[self.name.text stringByReplacingOccurrencesOfString:@" " withString:@""] identifyNumberQrought:[self.identity.text stringByReplacingOccurrencesOfString:@" " withString:@""] successCallbackQrought:successCallback failCallbackQrought:^(NSError *error) {
