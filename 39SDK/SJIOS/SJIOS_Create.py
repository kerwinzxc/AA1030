...
def filetext_replace(filepath, plugin_name):
    ...
    with open(filepath, "r") as f:
        filecontent = f.read()
        filecontent = re.sub(r'SJIOS', plugin_name, filecontent)
...
def rename_plugin(plugin_folder, plugin_name):
    for root, dirs, filenames in os.walk(plugin_folder):
        for filename in filenames:
            newfilename = re.sub("SJIOS", plugin_name, filename)
    ...
    for root, dirs, filenames in os.walk(plugin_folder, topdown=False):
        for dirname in dirs:
            newdirname = re.sub("SJIOS", plugin_name, dirname)
...
#    if not plugin_name.startswith('SJIOSContainer_'):
#        plugin_name = 'SJIOSContainer_' + plugin_name
    ...
    template_plugin_path = os.path.join(plugin_root, "SJIOS_Code")
