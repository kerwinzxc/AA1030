...
#import "PostinSdkImp.h"
@interface PostinChangePasswordViewController ()
...
@implementation PostinChangePasswordViewController
...
-(void)initNavigation {
    ...
    serviceImageView.image = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Postin.bundle/Postin_icon_meun_change_pw.png"]];
...
-(void)changePassword {
    ...
    if ([newPassword length] == 0) {
        PostinProgressHUD *process = [[PostinProgressHUD alloc] initWithView:self.view];
        ...
        @try
        {
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if([userinfo.tourist isEqualToString:@"1"]){
                    ...
                    nowPassword=[PostinSdkMd5 md5:self.nowPasswordField.text];
                }else{
                    
                    nowPassword=self.nowPasswordField.text;
                }
                
                [[[PostinDataBase alloc]init]updatePostinPassword:nowPassword userName:userinfo.username];
    ...
    if([userinfo.tourist isEqualToString:@"1"]){
        ...
        oldPassword = [PostinSdkMd5 md5:self.oldPasswordField.text];
    }
    
    [[PostinWebInterface sharedInstance]changePostinPasswordWithTokenPostin:self.accessToken oldPasswordPostin:oldPassword newPasswordPostin:[PostinSdkMd5 md5:newPassword] successCallbackPostin:successCallback failCallbackPostin:^(NSError *a) {
...
-(void)bindDespatch {
    ...
    [process showAnimated:YES whileExecutingBlock:^{
        ...
        while (self.changing)
        {
            ...
            [NSThread sleepForTimeInterval:1];
            if (count > 5) {
                PostinProgressHUD *errorHud = [[PostinProgressHUD alloc] initWithView:self.view];
