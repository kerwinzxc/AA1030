...
#import  "PostinSdkImp.h"
@interface PostinBindEmailViewController ()
...
@implementation PostinBindEmailViewController
...
-(void)initNavigation {
    ...
    serviceImageView.image = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Postin.bundle/Postin_icon_meun_bangemail.png"]];
...
- (void)normalPressed
{
    ...
    Postin_VSD_BLOCK successCallback = ^(NSDictionary *dictionary, NSDictionary *headers) {
     ...
     [[PostinWebInterface sharedInstance] bindPostinMail:self.accessToken emailPostin:verify successPostinCallback:successCallback failPostinCallback:^(NSError *error) {
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    if (textField == self.phoneNumberField)
    {
        ...
        if ([username length] == 0)
        {
            ...
            [[PostinSdkImp sharedInstance]showPostinToast:@"邮箱为空"];
