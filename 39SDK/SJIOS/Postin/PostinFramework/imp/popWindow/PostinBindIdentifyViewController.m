...
#import  "PostinSdkImp.h"
@interface PostinBindIdentifyViewController ()
...
@property(nonatomic,strong) PostinBindIdentifyViewController *bindIdentifyVew;


@end

@implementation PostinBindIdentifyViewController
...
-(void)initNavigation {
    ...
    serviceImageView.image = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Postin.bundle/Postin_icon_meun_shiming.png"]];
...
- (void)normalPressed
{
    ...
    Postin_VSD_BLOCK successCallback = ^(NSDictionary *dictionary, NSDictionary *headers) {
        @try
        {
            ...
            if ((NSNull *)value == nil)
            {
                ...
                [self showPostinToast:@"身份证信息有误"];
                return;
            }
            else
            {
                
                [self showPostinToast:[[PostinSdkImp sharedInstance] translatePostin:[dictionary valueForKey:@"error"]]];
            }
        }
        @catch(NSException * e)
        {
    ...
    [[PostinWebInterface sharedInstance] startPostinIdentify:self.accessToken namePostin:identifyNameField.text identifyNumberPostin:identifyNumberField.text successCallbackPostin:successCallback failCallbackPostin:^(NSError *error) {
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    ...
    else if(textField==identifyNumberField)
    {
        ...
        if ([username length] == 0)
        {
            ...
            [[PostinSdkImp sharedInstance]showPostinToast:@"姓名为空"];
        ...
        {
            [[PostinSdkImp sharedInstance]showPostinToast:@"身份证为空"];
