...
@property(nonatomic,strong) PostinForgetView *ymForgetView;//忘记密码找回界面
...
@implementation PostinForgetView
...
-(void)setLandScapeView{
    ...
    [bindPhoneFindPasswordButton setBackgroundImage:[UIImage imageNamed:@"Postin.bundle/Postin_login_button.png"] forState:UIControlStateNormal];
    ...
    [shensuFindPasswordButton setBackgroundImage:[UIImage imageNamed:@"Postin.bundle/Postin_login_button.png"] forState:UIControlStateNormal];
...
//    [shensuFindPasswordButton addSubview:imageView];
    ...
    [verifyButton setBackgroundImage:[UIImage imageNamed:@"Postin.bundle/Postin_login_button.png"] forState:UIControlStateNormal];
    ...
    newPasswordField.background = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Postin.bundle/Postin_register_edit_background.png"]];
...
-(void)setPortraitView{
    ...
    [sureButton setBackgroundImage:[UIImage imageNamed:@"Postin.bundle/Postin_login_button.png"] forState:UIControlStateNormal];
    [sureButton addTarget:self action:@selector(normalPressed) forControlEvents:UIControlEventTouchUpInside];
    sureButton.zhw_ignoreEvent = NO;//yes不启用防止重复点击，NO启用防止重复点击
    sureButton.zhw_acceptEventInterval = 3.0;//重复点击间隔时间
    [self.alertContainer addSubview:sureButton];
    self.alertContainer.backgroundColor=[UIColor colorWithPatternImage:[UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Postin.bundle/Postin_background_landscape.png"]]];
...
{
    Postin_VSD_BLOCK successBlock= ^(NSDictionary *dictionary, NSDictionary *header) {
        ...
        if ([dictionary objectForKey:@"error"] == nil) {
            ...
            PostinForgetEmailView *forgetView = [[PostinForgetEmailView alloc]initWithLandscape:self.landscape phone:self.phone qq:self.qq];
    ...
    [[PostinWebInterface sharedInstance]getPostinSupport:[[PostinSdkImp sharedInstance]getPostinAppId] onSuccessPostin:successBlock onFailedPostin:^(NSError *error) {
...
- (void)verifyPressed
{
    ...
    [[PostinWebInterface sharedInstance] changePostinPasswordByPhone:username
...
- (void)normalPressed
{
    ...
    if (username.length == 0)
    {
        PostinProgressHUD *myProcess = [[PostinProgressHUD alloc] initWithView:self.alertContainer];
        
        [self.alertContainer addSubview:myProcess];
        //process.dimBackground = YES;
        myProcess.mode = PostinProgressHUDModeCustomView;
...
//                process.labelText = [[PostinSdkImp sharedInstance] translatePostin:[dictionary valueForKey:@"error"]];
        ...
        [[PostinWebInterface sharedInstance] confirmchangePostinPasswordByPhone:username
...
//    }
    ...
    [process showAnimated:YES whileExecutingBlock:^{
        ...
        while (self.reseting)
        {
            ...
            [NSThread sleepForTimeInterval:1];
            if (count > 5) {
                PostinProgressHUD *errorHud = [[PostinProgressHUD alloc] initWithView:self.alertContainer];
                [self.alertContainer addSubview:errorHud];
                errorHud.labelText = @"网络不太好，请重试！！！";
                errorHud.mode = PostinProgressHUDModeCustomView;
