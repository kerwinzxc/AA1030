...
@property (nonatomic,strong) PostinUserInfo *loginInfo;
@property(nonatomic)BOOL login;//是否登录过
@property(nonatomic,strong)UIView *scroll;//用户列表
@property (nonatomic, strong) UITableView *myTableView;
@property (nonatomic,strong)UIButton *defaultButton;
@property(nonatomic,strong)  PostinRegisterView *registerView;//注册
@property(nonatomic,strong) PostinForgetView *forgetPhoneView;//手机找回密码界面
@property(nonatomic) Boolean SHOWPROCESS;
@property(nonatomic,strong) PostinBaseView *baseView;
@property(nonatomic,strong) PostinYMLoginView *ymLoginView;//登录界面
...
@implementation PostinYMLoginView
...
-(void)setLandScapeView{
    ...
    UIImage *backImage = [[UIImage alloc]initWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Postin.bundle/Postin_ym_back.png"]] ;
    ...
    UIImage *edit_up_image = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Postin.bundle/Postin_register_edit_background.png"]];
    ...
    UIImage * logo_r = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Postin.bundle/Postin_logo_r.png"]];
    ...
    password.background = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Postin.bundle/Postin_register_edit_background.png"]];
    ...
    UIImage * logo_s = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Postin.bundle/Postin_logo_s.png"]];
    ...
    UIImage *ymLoginBackImage = [[UIImage alloc]initWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Postin.bundle/Postin_login_button.png"]];
...
-(void) chooseClick{
    if(self.agree == true){
        self.agree = false;
        [self.choose setBackgroundImage:[UIImage imageNamed:@"Postin.bundle/Postin_unOK.png"] forState:
         UIControlStateNormal];
    }else{
        self.agree = true;
        [self.choose setBackgroundImage:[UIImage imageNamed:@"Postin.bundle/Postin_OK.png"] forState:
         UIControlStateNormal];
    }
}
...
- (void)judgeUserName
{
    ...
    if([password length]== 0){
        [[PostinSdkImp sharedInstance]showPostinToast:@"请输入密码"];
        return;
    }
    ...
    if ([password length]< 6 || [password length] > 20) {
        [[PostinSdkImp sharedInstance]showPostinToast:@"密码长度为6到20位"];
        return;
    }
    ...
    {
        [[PostinSdkImp sharedInstance]showPostinToast:@"用户名不能为空"];
        return;
    }

    if ([password length] == 0)
    {
        [[PostinSdkImp sharedInstance]showPostinToast:@"密码不能为空"];
...
-(void)ymStartLogin{
        ...
        @try{
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if(self.textIsphoneNumber){
                    [[PostinSdkImp sharedInstance] addPostinUser:username passwordPostin:password bindPostin:@"1" touristPostin:@"0" phoneNumberPostin:@"1"];
                }else{
                    [[PostinSdkImp sharedInstance] addPostinUser:username passwordPostin:password bindPostin:@"0" touristPostin:@"0" phoneNumberPostin:@"0"];
                }
    ...
    if(self.textIsphoneNumber){//手机号登录
        ...
        [[PostinWebInterface sharedInstance] phonePostinLogin:username
                                          passwordPostin:password
                                   successCallbackPostin:successCallback1 failCallbackPostin:^(NSError * error) {
                                       self.logining = false;
                                       self.failed(-109);
                                       [[PostinSdkImp sharedInstance]showPostinToast:@"请检查网络连接！"];
                                   }];
        ...
        [[PostinWebInterface sharedInstance] loginPostin:username
                                     passwordPostin:[PostinSdkMd5 md5:password]
...
- (void) startLogin
{
        ...
        @try{
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if(self.isRegister == YES && [[PostinSdkImp sharedInstance] getPostinShiMingRenZhengFlag] == YES){
                    [[PostinSdkImp sharedInstance] showPostinShiMingRenZhengView:self.token callbackPostin:^(NSString *callbackxxxx){
                        NSString * userid = [dictionary valueForKey:@"id"];
                        ...
                        if ([self.loginInfo.tourist isEqualToString:@"1"]&&[self.loginInfo.bind isEqualToString:@"0"]) {
                            ...
                            [[PostinSdkImp sharedInstance] showPostinToast:@"该帐号为快速登录帐号,请尽快绑定,以免帐号丢失!"];
                        }
                        [[PostinSdkImp sharedInstance] addPostinUser:self.loginInfo.username passwordPostin:self.loginInfo.password bindPostin:self.loginInfo.bind    touristPostin:self.loginInfo.tourist phoneNumberPostin:self.loginInfo.phoneNumber];
                        [self accountClick];
    ...
    //普通注册后的普通登录
        [[PostinWebInterface sharedInstance] loginPostin:self.loginInfo.username
                                     passwordPostin:[PostinSdkMd5 md5:self.loginInfo.password]
                              successCallbackPostin:successCallback
                                  failCallbackPostin:^(NSError * error) {
                                      self.login = NO;
                                      self.failed(-106);
...
-(void)ymdismissWithAnimation:(BOOL)animated
{
    ...
    [[PostinSdkImp sharedInstance] showPostinWelcome:[self.username text]];
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    ...
    else if(textField==self.password)
    {
        ...
        if ([username length] == 0)
        {
            ...
            [[PostinSdkImp sharedInstance]showPostinToast:@"用户名为空"];
        ...
        if ([password length] == 0)
        {
            [[PostinSdkImp sharedInstance]showPostinToast:@"密码为空"];
...
-(void) ymRegister{
    ...
    if (self.registerView == nil) {
        ...
        self.registerView = [[PostinRegisterView alloc] initWithBlock:^(NSString * username, NSString * password) {

            
            PostinUserInfo *userinfo=[[PostinUserInfo alloc]initWithInfo:username password:password bind:@"0" tourist:@"0" loginTime:0 phoneNumber:@"0"];
...
-(void)fogetPassword
{
    ...
    if (self.forgetPhoneView == nil) {
        self.forgetPhoneView = [[PostinForgetView alloc] initWithLandscape:self.landscape];
    }
