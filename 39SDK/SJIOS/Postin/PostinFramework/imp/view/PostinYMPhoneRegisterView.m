...
@property(nonatomic,strong)  PostinYMPhoneLogin *ymPhoneLogin;//注册
@property(nonatomic,strong) PostinYMPhoneRegisterView *ymPhoneView;//登录界面
...
@implementation PostinYMPhoneRegisterView
...
-(void)ym_phoneNextButton
{
        ...
        @try
        {
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if (ok != nil)
                {
                    ...
                    [[PostinSdkImp sharedInstance]showPostinToast:@"此号码未注册过，请注册登录"];
                    
                    
                         self.ymPhoneRegister = [[PostinYMPhoneRegister alloc]initWithBlock:^(NSString * username, NSString * password){
                                    PostinUserInfo *userinfo=[[PostinUserInfo alloc]initWithInfo:username password:password bind:@"1" tourist:@"0" loginTime:0 phoneNumber:@"1"];
                ...
                [[PostinSdkImp sharedInstance]showPostinToast:@"此号码已经注册过，请输入密码直接登录"];
     
                if (self.ymPhoneLogin==nil) {
                    self.ymPhoneLogin=[[PostinYMPhoneLogin alloc]initWithBlock:self.success failed:self.failed landScape:self.landscape view:self.quidklogin ymPhoneNumber:self.ymPhoneNum.text];
    ...
    [[PostinWebInterface sharedInstance] checkPostinPhoneRegist:self.ymPhoneNum.text
...
- (BOOL)textFieldShouldReturn:(UITextField *)textField
{
    if(textField==self.username)
    {
        ...
        if ([username length] == 0)
        {
            ...
            [[PostinSdkImp sharedInstance]showPostinToast:@"请输入正确的手机号"];
...
- (void) startLogin
{
            ...
            if ((NSNull *)value == nil)
            {
                ...
                if(self.isRegister == YES && [[PostinSdkImp sharedInstance] getPostinShiMingRenZhengFlag] == YES){
                    [[PostinSdkImp sharedInstance] showPostinShiMingRenZhengView:self.token callbackPostin:^(NSString *callbackssss){
                        ...
                        [[PostinSdkImp sharedInstance] addPostinUser:self.loginInfo.username passwordPostin:self.loginInfo.password bindPostin:@"1" touristPostin:@"0" phoneNumberPostin:@"1"];
    ...
    Postin_VSD_BLOCK successCallback = ^(NSDictionary *dictionary, NSDictionary *headers) {
        ...
        [[PostinWebInterface sharedInstance] phonePostinLogin:self.ymPhoneNum.text
                                          passwordPostin:self.loginInfo.password
                                   successCallbackPostin:successCallback failCallbackPostin:^(NSError * error) {
            self.login = NO;
            self.failed(-106);
            [[PostinSdkImp sharedInstance]showPostinToast:@"请检查网络连接！"];
        }];
    ...
    [self processPostinShowLogin];
}

-(void)processPostinShowLogin {
    if (self.process == nil) {
        self.process = [[PostinProgressHUD alloc] initWithView:self.alertContainer];
