...
#import "PostinYMPhoneRegister.h"
#import "PostinYMPhoneRegisterView.h"
#import "PostinProgressHUD.h"


#define Postin_USER_AGREEMENT_URL @"https://www.3975.com/public/web/39XY.html"
@interface PostinYMPhoneRegister()
...
@property(nonatomic,strong)  PostinYMPhoneRegister *ymPhoneRegister;//注册
@property(nonatomic,strong)  PostinYMPhoneRegisterView *ymPhoneRegisterView;//注册
...
@property (nonatomic, strong)PostinProgressHUD *myProcess;
...
@implementation PostinYMPhoneRegister
...
-(void)setLandScapeView{
    ...
    UIImage * logo_r = [UIImage imageWithContentsOfFile:[[[NSBundle mainBundle]resourcePath]stringByAppendingPathComponent:@"Postin.bundle/Postin_logo_s.png"]];
...
-(void)setPortraitView{
    ...
    ymloginButton.zhw_ignoreEvent = NO;//yes不启用防止重复点击，NO启用防止重复点击
    ymloginButton.zhw_acceptEventInterval = 3.0;//重复点击间隔时间
    [ymloginButton setBackgroundImage:[UIImage imageNamed:@"Postin.bundle/Postin_login_button.png"] forState:UIControlStateNormal];
    [ymloginButton addTarget:self action:@selector(ym_phoneRegisterPressed) forControlEvents:UIControlEventTouchUpInside];
...
-(void)ym_verifyPressed:(id)sender {
    ...
    Postin_VSD_BLOCK successCallback = ^(NSDictionary *dictionary, NSDictionary *headers) {
        ...
        @try
        {
            ...
            if ((NSNull *)value == nil)
            {
                ...
                [[PostinSdkImp sharedInstance]showPostinToast:@"获取验证码成功"];
                self.logining = false;
            ...
            {
                self.myProcess = [[PostinProgressHUD alloc] initWithView:self.alertContainer];
                [self.alertContainer addSubview:self.myProcess];
                [self.alertContainer bringSubviewToFront:self.myProcess];
                self.myProcess.labelText = [[PostinSdkImp sharedInstance] translatePostin:[dictionary valueForKey:@"error"]];
                self.myProcess.mode = PostinProgressHUDModeCustomView;
    ...
    [[PostinWebInterface sharedInstance] phonePostinRegistVerify:self.ymPhoneNumber
...
//                [self setRandomAccount];
    ...
    [[PostinWebInterface sharedInstance] phonePostinRegist:self.ymPhoneNumber
                                            passwordPostin:self.password.text
                                          verifyCodePostin:self.verifyField.text
                                     successCallbackPostin:successCallback
                                         failCallbackPostin:^(NSError * error) {
                                             self.registering = false;
...
//        [alertView show];
        ...
        [[PostinSdkImp sharedInstance] showPostinToast:@"注册成功！帐号信息截图已保存到相册中，帐号丢失可以查看截图找回，请妥善保管！"];
    }else{
        NSLog(@"save error");
...
-(void)dismissWithAnimation:(BOOL)animated
{
    ...
    [[PostinWebInterface sharedInstance] PostinsendHeart:self.code];
